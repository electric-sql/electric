...
async function init() {
  ...
  const builderFile = path.join(__dirname, 'builder.cjs')
  ...
  await findAndReplaceInFile(
    `postgresql://electric-generator:proxy_password@localhost:${oldElectricProxyPort}/electric`,
    `postgresql://electric-generator:proxy_password@localhost:${electricProxyPort}/electric`, packageJsonFile
  )
  ...
  const startElectricFile = path.join(__dirname, 'backend', 'startElectric.cjs')
