...
export default function PGliteMigrateModal({ isOpen, onDismiss }: Props) {
  ...
  const [postgresqlVersion, setPGVersion] = useState<string | undefined>(
    undefined
  )
  ...
  const migrate = async () => {
    ...
    const currentVersion = await pg.query<{ version: string }>(
      'SELECT version();'
    )
    ...
    >
      <div className="flex flex-col w-full h-100 p-4">
        <h3 className="text-lg font-semibold mb-2">
          This is demo of migrating a PGlite database via pg_dump.
        </h3>
        <div className="mb-6">
