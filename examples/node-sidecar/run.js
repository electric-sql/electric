...
const main = async () => {
  // install deps
  await exec('yarn', [], appPath)
  // shutdown the backend if it's still up from a previous run
  //await exec('yarn', ['backend:stop'], appPath)
  // start the backend
  await exec('yarn', ['backend:up'], appPath)
  // Give the backend time to start up
  await sleep(2000)
  // migrate the database
  await exec('yarn', ['db:migrate'], appPath)
  ...
  exec('yarn', ['start', 'config.json'], sidecarPath).catch(() => {
    // it's normal that this one may throw
    // because we kill it later when the app is done
  })
