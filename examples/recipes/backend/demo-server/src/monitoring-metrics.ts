...
function simulateHighCpuLoadOverTime({
  ...
  const simulate = () =>
    setTimeout(
      () => {
        lastCpuLoadDurationMs = Math.max(minLoadTimeMs, Math.random() * maxLoadTimeMs)
        simulateHighCpuLoad(lastCpuLoadDurationMs)
      },
      lastCpuLoadDurationMs +
        Math.max(minTimeBetweenLoadsMs, Math.random() * maxTimeBetweenLoadsMs),
    )
  simulate()
}
