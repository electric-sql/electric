...
__electric__="$(cd "${__dir__}/../.."; pwd)"
__src__="${__dir__}/src-tauri"
...
build_electric() {
    ...
    (
        cd "${__electric__}/clients/typescript"
        pnpm install
        pnpm build
    )
    (
        cd "${__electric__}/generator"
        pnpm install
        pnpm build
    )
}
...
git_clone_third_parties() {
    # We are in root
    mkdir -p "${__src__}/crates"
    cd "${__src__}/crates"

    if [[ ! -d ${__src__}/crates/ollama-rs ]]; then
        git clone https://github.com/pepperoni21/ollama-rs "${__src__}/crates/ollama-rs" # Solves a bug where in Cargo.toml `git` would still not work as expected
    fi

    cd "${__src__}/crates/ollama-rs"
    git checkout f610472689ec113689ab06fb58304ec723c93111

    # remove any existing pg-embed and start from scratch, otherwise the patch below won't apply
    rm -rf "${__src__}/crates/pg-embed"
    git clone https://github.com/faokunega/pg-embed "${__src__}/crates/pg-embed"
    # We need to modify this in order to have a different location for postgres
    cd "${__src__}/crates/pg-embed"
    git apply "${__dir__}/pg-embed.patch"
}
...
install_ollama() {
    ...
    curl -L -o ${__src__}/ollama-linux-amd64 https://github.com/ollama/ollama/releases/download/v0.1.22/ollama-linux-amd64
    chmod +x ${__src__}/ollama-linux-amd64

    # Tauri needs this specific name
    mv ${__src__}/ollama-linux-amd64 ${__src__}/ollama-x86_64-unknown-linux-gnu
}
...
install_postgres() {
    ...
    git clone --branch v0.5.1 https://github.com/pgvector/pgvector.git "${__src__}/pgvector"
    # cd pgvector
    ...
    mv "${__src__}/pgvector/sql/vector.sql" "${__src__}/pgdir/share/postgresql/extension/vector--0.5.1.sql"
    mv "${__src__}/pgvector/vector.control" "${__src__}/pgdir/share/postgresql/extension/vector.control"
    # mv pgvector/vector.so ../src-tauri/pgdir/lib/postgresql/vector.so
    # We don't need pgvector sources from this point
    rm -rf "${__src__}/pgvector"
}
...
install_onnxruntime() {
    # We are in root
    cd "${__src__}"
    curl -L -O https://github.com/microsoft/onnxruntime/releases/download/v1.16.3/onnxruntime-linux-x64-1.16.3.tgz
    tar -xzvf onnxruntime-linux-x64-1.16.3.tgz
    rm onnxruntime-linux-x64-1.16.3.tgz
    cp onnxruntime-linux-x64-1.16.3/lib/libonnxruntime.so.1.16.3 "${__src__}/libonnxruntime.so"
