 ...
 */
export function generateOplogTriggers(tableFullName: TableFullName, table: Omit<Table, 'foreignKeys'>): Statement[] {
 ...
 */
function generateCompensationTriggers(tableFullName: TableFullName, table: Table, tables: Tables): Statement[] {
  ...
  const makeTriggers = (foreignKey: ForeignKey) => {
    ...
    if (fkTable === undefined) throw new Error(`Table ${foreignKey.table} for foreign key not found.`)
 ...
 */
export function generateTableTriggers(tableFullName: TableFullName, tables: Tables): Statement[] {
  const table = tables.get(tableFullName)
  if (typeof table === 'undefined')
    throw new Error(`Could not generate triggers for ${tableFullName}. Table not found.`)
