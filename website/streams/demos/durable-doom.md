...
Doom on [Durable&nbsp;Streams](/streams/). Every game tic is an offset, every offset is a branch point. Live spectating, time-travel, and [fork](/blog/2026/04/15/fork-branching-for-durable-streams)-to-continue — all client-side, no backend.

<DemoCTAs :demo="$frontmatter" />

## How it works

Doom's game engine is compiled to WebAssembly and runs entirely in the browser. Every tic (input frame) is captured and appended to a [Durable&nbsp;Stream](/streams/). Because Doom is a deterministic pure function of its input log, replaying the stream reproduces identical game state.
