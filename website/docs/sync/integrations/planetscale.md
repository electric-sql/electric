...
For general PostgreSQL user and permission setup, see the [PostgreSQL Permissions guide](/docs/sync/guides/postgres-permissions).
...
Small PlanetScale clusters may start with a low `max_connections` limit. Electric uses **20 connections** for its connection pool by default (configurable via [`ELECTRIC_DB_POOL_SIZE`](/docs/sync/api/config#electric-db-pool-size)).
...
> 3. **Use Manual Mode** — set `ELECTRIC_MANUAL_TABLE_PUBLISHING=true` and have an admin pre-configure the publication and replica identity. See [Manual Mode](/docs/sync/guides/postgres-permissions#manual-mode-setup) in the PostgreSQL Permissions guide.

## Database User Setup

Follow the [PostgreSQL Permissions guide](/docs/sync/guides/postgres-permissions) to set up the Electric user with proper permissions.
...
> PlanetScale requires tables to be [added to publications individually](https://planetscale.com/docs/postgres/integrations/logical-cdc) and does not support `CREATE PUBLICATION ... FOR ALL TABLES`. If you encounter this issue, explicitly list tables in your publication. See [Manual Publication Management](/docs/sync/guides/postgres-permissions#manual-configuration-steps) in the PostgreSQL Permissions guide.

## Troubleshooting

For general PostgreSQL permission and configuration errors, see the [PostgreSQL Permissions guide](/docs/sync/guides/postgres-permissions#troubleshooting).
...
**Solution:** Run your migrations as the same database user that Electric connects as. See [Table Ownership](#table-ownership) for details, or use [Manual Mode](/docs/sync/guides/postgres-permissions#manual-mode-setup) (`ELECTRIC_MANUAL_TABLE_PUBLISHING=true`) if you cannot re-run migrations.
...
- [PostgreSQL Permissions guide](/docs/sync/guides/postgres-permissions)
- [Deployment guide](/docs/sync/guides/deployment)
...
- [PostgreSQL Permissions Guide](/docs/sync/guides/postgres-permissions)
- [Electric Deployment Guide](/docs/sync/guides/deployment)
