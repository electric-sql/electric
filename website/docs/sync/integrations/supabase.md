...
Configure Electric to connect to the direct access `DATABASE_URL` you copied above. Set [`ELECTRIC_DATABASE_USE_IPV6`](/docs/sync/api/config#database-use-ipv6) to `true`, e.g.:
...
Electric and Supabase are also collaborating to develop [PGlite](/sync/pglite), which Supabase sponsor, contribute to and have developed [database.build](https://database.build) on.
