...
**→ See the [authentication guide](/docs/sync/guides/auth) for complete proxy implementation with streaming, error handling, and authorization.**

### `useShape`

[`useShape`](https://github.com/electric-sql/electric/blob/main/packages/react-hooks/src/react-hooks.tsx#L131) binds a materialised [Shape](/docs/sync/api/clients/typescript#shape) to a state variable.
...
`useShape` takes the same options as [ShapeStream](/docs/sync/api/clients/typescript#options). The return value is a `UseShapeResult`:
...
It takes the same options as [ShapeStream](/docs/sync/api/clients/typescript#options).
