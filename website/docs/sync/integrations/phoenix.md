...
The APIs map [Ecto queries](https://hexdocs.pm/ecto/Ecto.Query.html) to [Shapes](/docs/sync/guides/shapes).
...
You can sync _into_ any client in any language that [speaks HTTP and JSON](/docs/sync/api/http). For example, using the Electric [Typescript client](/docs/sync/api/clients/typescript):
...
Or binding a shape to a component using the [React bindings](/docs/sync/integrations/react):
...
2. `:http` where Electric does _not_ need to be included as an application dependency and Phoenix.Sync consumes data from an external Electric service using it's [HTTP API](/docs/sync/api/http)
...
The main Electric monorepo has a [Phoenix LiveView example](/sync/demos/phoenix-liveview). This is an example Phoenix LiveView application that uses [`Electric.Phoenix.LiveView.sync_stream/4`](https://hexdocs.pm/phoenix_sync/Phoenix.Sync.LiveView.html#sync_stream/4) to sync data from Postgres into a LiveView using [Phoenix Streams](https://fly.io/phoenix-files/phoenix-dev-blog-streams/).

### Gatekeeper Auth

The [Gatekeeper auth](/sync/demos/gatekeeper-auth) example also contains a Phoenix application that uses Plug to authorize shape access and issue shape-scoped access tokens.
