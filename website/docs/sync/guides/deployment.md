...
Electric connects to your Postgres using a `DATABASE_URL`. Your app connects to Electric [over HTTP](/docs/sync/api/http), usually using a [Client library](/docs/sync/api/clients/typescript).
...
You also often want to proxy requests to Electric through your API, or other proxy. For example, to implement [auth](./auth) and/or [caching](/docs/sync/api/http#caching). In these cases, you'll also need to deploy your API and/or proxy layer in front of Electric.
...
See the [Security guide](/docs/sync/guides/security) for information.
...
- [Supabase](/docs/sync/integrations/supabase)
- [Neon](/docs/sync/integrations/neon)
- [AWS (RDS and Aurora)](/docs/sync/integrations/aws)
- [GCP (Cloud SQL and Alloy)](/docs/sync/integrations/gcp)
- [Digital Ocean](/docs/sync/integrations/digital-ocean)
- [Crunchy](/docs/sync/integrations/crunchy)
...
You connect to Postgres using a [`DATABASE_URL`](/docs/sync/api/config#database-url) env var. This connection string contains your user credentials and an `sslmode` parameter.
...
You can optionally provide a separate [`ELECTRIC_POOLED_DATABASE_URL`](/docs/sync/api/config#electric-query-database-url) env var, which can use a pooler and will be used for all queries other than replication.

To force Electric to verify the database server's certificate when connecting to it using TLS, set the [`ELECTRIC_DATABASE_CA_CERTIFICATE_FILE`](/docs/sync/api/config#electric-database-ca-certificate-file) config option.

> [!Tip] Troubleshooting common errors
> If you get a TCP connection error saying `non-existing domain - :nxdomain` or `network is unreachable - :enetunreach` then you may need to connect using IPv6. You can enable this by setting [`ELECTRIC_DATABASE_USE_IPV6=true`](/docs/sync/api/config#database-use-ipv6).
>
> If you get a TCP connection `timeout` error then make sure you're connecting directly to Postgres and not via a connection pool. For example, when using [Supabase](/docs/sync/integrations/supabase) you need to untick their "Use connection pooling" option on the database settings page.
>
...
- [AWS](/docs/sync/integrations/aws)
- [GCP](/docs/sync/integrations/gcp)
- [Digital Ocean](/docs/sync/integrations/digital-ocean)
...
Electric provides an HTTP API exposed on a configurable [`ELECTRIC_PORT`](/docs/sync/api/config#electric-port). You should make sure this is exposed to the Internet.

### Health checks

Electric provides a health check endpoint at `/v1/health` that can be used for liveness and readiness probes. This endpoint does not require authentication, so it works even when [`ELECTRIC_SECRET`](/docs/sync/api/config#electric-secret) is set.
...
> If you are performing rolling deployments with `maxSurge: 1`, the `exec` probe above will cause a deadlock &mdash; the new pod can never return `200` while the old pod holds the replication lock. Use an `httpGet` readiness probe instead, which accepts any 2xx. See the [Upgrading guide](/docs/sync/guides/upgrading) for details.
...
See the [Caching section](/docs/sync/api/http#caching) of the HTTP API docs for more information.

### Upgrading

If you're running Electric behind an orchestrator that performs rolling updates (e.g., Kubernetes, AWS ECS), see the [Upgrading guide](/docs/sync/guides/upgrading) for strategies to minimize disruption when deploying new versions.

## 3. Connecting your app

You can then connect your app to Electric [over HTTP](/docs/sync/api/http). Typically you use a [Client library](/docs/sync/api/clients/typescript) and configure the URL in the constructor, e.g.:
...
You can connect to Electric from any language/environment that speaks HTTP. See the [HTTP API](/docs/sync/api/http) and [Client docs](/docs/sync/api/clients/typescript) for more information.
