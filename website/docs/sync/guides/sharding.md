...
|--------|---------|---------|
| [`DATABASE_URL`](/docs/sync/api/config#database-url) | Connection to this shard's Postgres | `postgresql://...@shard-0/db` |
| [`ELECTRIC_INSTANCE_ID`](/docs/sync/api/config#electric-instance-id) | Unique identifier for telemetry | `electric-shard-0` |
| [`ELECTRIC_REPLICATION_STREAM_ID`](/docs/sync/api/config#replication-stream-id) | Unique suffix for replication slot/publication | `shard-0` |
| [`ELECTRIC_STORAGE_DIR`](/docs/sync/api/config#electric-storage-dir) | Persistent storage path | `/data/shard-0` |
...
> The proxy above handles **shard routing** but does not enforce **authorization**. In production, your proxy should also validate the user's identity and set an appropriate `where` clause to restrict data access. See [Combining with auth](#combining-with-auth) below and the [Auth guide](/docs/sync/guides/auth) for complete examples.
...
Each Electric instance exposes metrics via [OpenTelemetry](/docs/sync/reference/telemetry). Configure each instance with a unique `ELECTRIC_INSTANCE_ID` to distinguish metrics per shard in your observability platform.
...
3. Clients receive a [`must-refetch`](/docs/sync/api/http#control-messages) control message from the old shard
...
- Review the [deployment guide](/docs/sync/guides/deployment) for production configuration
- See the [upgrading guide](/docs/sync/guides/upgrading) for rolling deployment strategies
- See [auth patterns](/docs/sync/guides/auth) for securing your sharded deployment
- Check [benchmarks](/docs/sync/reference/benchmarks) for performance expectations per shard
