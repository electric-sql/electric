...
The golden rule with Electric is that it's [all just HTTP](/docs/sync/api/http).

So when it comes to auth, you can use existing primitives, such as your API, middleware and external authorization services<!-- (like [Auth0](/docs/sync/integrations/auth0) and [Authzed](/docs/sync/integrations/auth0)) -->.

### Shapes are resources

With Electric, you sync data using [Shapes](/docs/sync/guides/shapes) and shapes are just resources.

You access them by making a request to `GET /v1/shape`, with the [shape definition](/docs/sync/guides/shapes#defining-shapes) in the query string (`?table=items`, etc.). You can authorise access to them exactly the same way you would any other web resource.
...
There's no need to use database rules to [secure data access](/docs/sync/guides/security) when your sync engine runs over standard HTTP.
...
1. add an `Authorization` header to your [`GET /v1/shape`](/docs/sync/api/http#syncing-shapes) request
...
When using the [Typescript client](/docs/sync/api/clients/typescript), you can pass in a [`headers` option](/docs/sync/api/clients/typescript#options) to add an `Authorization` header.
...
The API is an [Elixir/Phoenix](/docs/sync/integrations/phoenix) web application that [exposes](https://github.com/electric-sql/electric/blob/main/examples/gatekeeper-auth/api/lib/api_web/router.ex) two endpoints:
...
See the [./client](https://github.com/electric-sql/electric/tree/main/examples/gatekeeper-auth/client) for an example using the [Typescript client](/docs/sync/api/clients/typescript) with gatekeeper and proxy endpoints:
...
Note: 5xx server errors are automatically retried with exponential backoff. See the [TypeScript client error handling docs](/docs/sync/api/clients/typescript#error-handling) for complete details.
