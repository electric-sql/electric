...
Electric writes [shape logs](/docs/sync/api/http#shape-log) to disk.
...
You can remove [`STORAGE_DIR`](/docs/sync/api/config#storage-dir) to delete all shape logs. This will ensure that following shape requests will be re-synced from scratch.
...
**Put a CDN in front of Electric** (recommended). Electric's caching headers are designed for CDN [request collapsing](/docs/sync/api/http#collapsing-live-requests). When multiple clients poll the same shape at the same offset, the CDN collapses them into a single request to Electric and fans out the response. This dramatically reduces concurrent connections. See the [deployment guide](/docs/sync/guides/deployment) for CDN setup.

**Increase the concurrent request limit** as a stopgap. Set [`ELECTRIC_MAX_CONCURRENT_REQUESTS`](/docs/sync/api/config#electric-max-concurrent-requests) to raise the limits:
...
4. **Check Electric's storage** &mdash; if [`ELECTRIC_STORAGE_DIR`](/docs/sync/api/config#electric-storage-dir) has disk space or permission issues, Electric can't flush data and won't acknowledge progress
...
Electric exposes metrics for monitoring replication slot health. If you have [Prometheus configured](/docs/sync/api/config#electric-prometheus-port), watch these metrics:
...
See the [PostgreSQL Permissions guide](/docs/sync/guides/postgres-permissions) for detailed instructions on:
...
**Option 1:** Set [`ELECTRIC_SHAPE_DB_EXCLUSIVE_MODE=true`](/docs/sync/api/config#electric-shape-db-exclusive-mode) to force SQLite to use a single read-write connection instead of multiple reader connections. This avoids locking issues at the cost of reduced read throughput.

**Option 2:** Set [`ELECTRIC_SHAPE_DB_STORAGE_DIR`](/docs/sync/api/config#electric-shape-db-storage-dir) to a local (non-shared) path. This keeps the SQLite database on local storage while shape logs remain on the shared network filesystem. The SQLite database will be rebuilt from the shape logs on startup.

### Replication slot recreation &mdash; why are all clients resyncing after a crash?

When Electric's replication slot is dropped or lost &mdash; whether due to a crash, use of [temporary replication slots](/docs/sync/guides/upgrading#temporary-replication-slots), or Postgres invalidating it because [`max_slot_wal_keep_size`](#recommended-postgresql-settings) was exceeded &mdash; the new slot starts from the current WAL position with no history.
...
If you're seeing `409` errors during deploys with shared storage, check that both instances are pointing to the same `ELECTRIC_STORAGE_DIR` on a shared filesystem. With [separate storage](/docs/sync/guides/upgrading#separate-storage-ephemeral), `409`s during deploys are expected since each instance has its own shape handles.
...
In order for Electric to connect to Postgres over IPv6, you need to set [`ELECTRIC_DATABASE_USE_IPV6`](/docs/sync/api/config#database-use-ipv6) to `true`.
...
By default Electric only binds to IPv4 addresses. You need to set [`ELECTRIC_LISTEN_ON_IPV6`](/docs/sync/api/config#electric-use-ipv6) to `true` to bind to bind to IPv6 addresses as well.
...
See the [HTTP API documentation](/docs/sync/api/http#subset-snapshots) for more details.
