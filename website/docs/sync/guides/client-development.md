...
The HTTP API exposes [Shapes](/docs/sync/guides/shapes). There are two phases to syncing a shape:
...
Encode a [shape definition](/docs/sync/guides/shapes#defining-shapes) into a `GET /v1/shape` URL. See the <a href="/openapi.html#/paths/~1v1~1shape~1%7Broot_table%7D/get">specification for the URL structure here</a>. For example, a Shape that contains all of the rows in the `items` table would be requested with:
...
The first request to a shape should set the `offset` parameter to `-1`. This indicates to Electric that you want to consume all of the data from the beginning of the [Shape log](/docs/sync/api/http#shape-log). For example, you might make a request to:
...
> Consuming data over HTTP allows us to [leverage CDNs](/docs/sync/api/http#caching), simplifies observability and allows you to implement auth (and other capabilities) [using HTTP proxies](/docs/sync/guides/auth#recommended-pattern).
...
If the previous response contains an `electric-cursor` header, then also set the `cursor` parameter to its value. (This is an extra cache-busting parameter used to normalise [request-collapsing](/docs/sync/api/http#collapsing-live-requests) behaviour across different CDNs).
...
If you just want a stream of logical database operations, you can simply stream or broadcast these onwards. This is what both the Typescript client [`ShapeStream`](/docs/sync/api/clients/typescript#shapestream) class and Elixir client [`stream/3`](/docs/sync/api/clients/elixir#stream) function do.

### Into a data structure

If you want to maintain a materialised Shape in your client, you can apply the operations in the shape log to a data structure. This is what both the Typescript client [`Shape`](/docs/sync/api/clients/typescript#shape) class and [Redis example](/sync/demos/redis) do.
...
If you maintain a materialised data structure, it's often useful to know when it changes. This is what the Typescript client's [`Shape.subscribe`](/docs/sync/api/clients/typescript#shape) function enables, for example.
...
Make sure you [have Electric running](/docs/sync/guides/installation) and then:
