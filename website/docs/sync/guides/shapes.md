...
> [!Warning] TanStack DB
> You can use shapes with [TanStack DB](/sync/tanstack-db) for expressive, [query-driven sync](https://tanstack.com/blog/tanstack-db-0.5-query-driven-sync).
...
Subqueries require enabling feature flags. Set `ELECTRIC_FEATURE_FLAGS=allow_subqueries,tagged_subqueries` to enable. See the [configuration docs](/docs/sync/api/config#allow_subqueries) for details.
:::
...
These requests both return an array of [Shape Log](/docs/sync/api/http#shape-log) entries. You can process these manually, or use a higher-level client.

### Typescript

You can use the [Typescript Client](/docs/sync/api/clients/typescript) to process the Shape Log and materialised it into a `Shape` object for you.
...
Or you can use framework integrations like the [`useShape`](/docs/sync/integrations/react) hook to automatically bind materialised shapes to your components.

See the [Quickstart](/docs/sync/quickstart) and [HTTP API](/docs/sync/api/http) docs for more information.
...
See the [TypeScript client docs](/docs/sync/api/clients/typescript#requesting-subset-snapshots) and [HTTP API docs](/docs/sync/api/http#subset-snapshots) for more details.
:::
...
Electric evaluates [where clauses](#where-clause) when processing changes from Postgres and matching them to [shape logs](/docs/sync/api/http#shape-log). If there are lots of shapes, this means we have to evaluate lots of where clauses. This has an impact on data throughput.
...
For more details see the [benchmarks](/docs/sync/reference/benchmarks#_7-write-throughput-with-optimized-where-clauses) and [this blog post](/blog/2025/08/13/electricsql-v1.1-released) about our storage engine.
