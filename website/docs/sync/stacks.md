...
[Electric](/docs/sync/guides/deployment#_2-running-electric) always runs as a service in front of [Postgres](/docs/sync/guides/deployment#_1-running-postgres), syncing into a [Client](/docs/sync/guides/shapes#subscribing-to-shapes) process or store, via a [Proxy](/docs/sync/guides/auth#requests-can-be-proxied) or backend API.
...
You can learn more about these by following <!-- the Tutorial and -->[Deployment](/docs/sync/guides/deployment) guide.
...
Electric works with [any Postgres with logical replication](/docs/sync/guides/deployment#_1-running-postgres) enabled. [Neon](/docs/sync/integrations/neon), [Supabase](/docs/sync/integrations/supabase) and [Crunchy](/docs/sync/integrations/crunchy) are all great choices for a Postgres host.

### Hosting your proxy

You can proxy requests to Electric either [through your backend API](/docs/sync/guides/auth#it-s-all-http), or through a cloud worker. [Cloudflare](/docs/sync/integrations/cloudflare) is a great choice for hosting workers because it only charges for actual processing time (not for wall clock time holding [sync connections](/docs/sync/api/http#live-mode) open).

### Other stacks

The stacks on this page are just some options and recommendations. You can use Electric with any&nbsp;technology you like &mdash; as long as it speaks [HTTP&nbsp;and&nbsp;JSON](/docs/sync/guides/client-development).
...
- [LinearLite demo](/sync/demos/linearlite) using PGlite with Electric
...
This works by exposing a [Shape](/docs/sync/guides/shapes) to sync changes for a [Y.Doc](https://docs.yjs.dev/api/y.doc). The `y-electric` package then automatically shares updates across all connected clients.

#### More information

- [Integration docs](/docs/sync/integrations/yjs)
- [Notes demo](/sync/demos/notes)
