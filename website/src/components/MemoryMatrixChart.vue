...
export default {
  props: {
    ...
    data: {
      type: Array,
      required: true,
      // Each item should have: { label: string, data: number[], color?: string }
    },
    labels: {
      type: Array,
      required: true,
...
<template>
  <div class="MemoryMatrixChart" ref="wrapperEl" :style="wrapperStyle">
    <h3>{{ title }}</h3>
...
.MemoryMatrixChart .chart-canvas {
  position: relative;
  flex: 1 1 auto;
  min-height: 0;
}
...
.MemoryMatrixChart .chart-subtitle {
  text-align: center;
  margin: 0 0 0.5rem;
