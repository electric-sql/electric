...
- Yjs and CRDTs are the established way to build collaborative editors — mature, well-understood, battle-tested
...
<!-- This section establishes the architectural foundation. The reader
     ...
     specifics. Keep it concrete — what are the two streams, how do
     they connect. -->
...
- The Collaborative AI Editor uses two Durable Streams per document: one for the Yjs document state (CRDT updates + awareness), one for the TanStack AI chat session
...
<!-- ASSET: Architecture diagram showing the two durable streams (Yjs doc
     + AI chat) flowing through the same Durable Streams infrastructure
     to the browser and server agent -->
...
- The hard problem: the AI generates text token by token, but you can't just append characters to a CRDT one at a time — that creates massive CRDT overhead and a jittery editing experience
- Three techniques solve this: document tools, relative position anchors, and stable prefix commits
...
- **snapshot** — reads the current document content so the agent can see what's in the doc before acting
- **search_and_replace** — find text and replace it, working at the Yjs level so concurrent edits don't break the operation
- **insert_at_cursor** — insert content at the user's current cursor position, resolved via relative anchors
...
- ProseMirror uses absolute integer positions — but those shift when other users type, making them useless for concurrent editing
- Yjs relative positions are anchored to CRDT items, not offsets — they stay correct regardless of concurrent edits
- The client encodes its cursor/selection as base64 relative anchors and sends them to the server via the chat context
...
- `takeStablePrefix` buffers the streaming token output and commits at natural boundaries — whitespace, punctuation, or when the buffer exceeds ~96 characters
- This batches CRDT updates into meaningful chunks rather than per-character operations, keeping the Yjs update size manageable
- The uncommitted tail is held in awareness state so the UI can optionally show a preview of what's coming

### Streaming markdown into ProseMirror

- The AI generates markdown, but ProseMirror needs structured nodes — paragraphs, headings, lists, marks
- A `streaming-markdown` pipeline incrementally parses the markdown stream and converts it to ProseMirror transactions
- Each stable chunk is committed as structured ProseMirror nodes — not raw text that gets re-parsed later
- The result: the AI types formatted content into the document in real-time, just like a human would
...
angle: "Two streams, one primitive: building a collaborative AI editor".
...
- [ ] Architecture diagram — two durable streams to browser + server agent
