...
The main principle behind its performance is to store bytes on disk in the format they are sent over the wire. A read is then a byte-range over a file that can be completely served on the kernel side, and a write is an append whose principal cost is durability. The rest of this post is how both are made inexpensive.
...
You can one-shot a naive implementation of durable streams. The challenge is making it scale to a large number of streams, with high throughput, low memory usage and without losing a single byte. Our Rust implementation does this while running an order of magnitude faster than our previous Node implementation.
...
Writes take data from the HTTP socket buffer and append it to a stream log file on disk. Once written to the file system, the bytes stay briefly in the page cache; any live subscribers waiting for new data on that stream are woken and fetch data at the newly written offset. Typically they find the data in the page cache, and the operation returns quickly for everyone, without copying the data.
...
Every WAL record carries a CRC32C checksum for recoverability. CRC32C is a hardware instruction on modern x86 and ARM CPUs and is very fast. Computing the checksum requires moving data into user space, which prevents using the `splice` optimization to copy data directly from the socket to the file. This is a cost paid once; because data is stored in wire format, reads remain fast.
...
**Catch-up**: Large reads stay bounded the same way: history streams out in fixed windows, so replaying a multi-gigabyte backlog costs about one window of memory rather than the full size. With no garbage collector, that footprint holds steady under load.

#### Tiered storage

Once the log's tail prefix reaches a certain length, the server seals the log *chunk* and offloads it to an object store, keeping only the active tail locally. Sealed chunks are immutable and carry long-lived cache headers, so repeated cold reads are served from a CDN and do not reach the origin.

### Comparison with Kafka

Kafka takes a different path to append throughput: it relies on [replication](https://kafka.apache.org/35/configuration/topic-level-configs/) for durability, only flushing to disk asynchronously. This server keeps fsync-based durability on a single node and makes it cheap with the WAL and group commit. The tradeoff is in what each guarantee covers: a single-node fsync protects against process and power loss on that node, whereas Kafka's replication also survives the loss of the disk or the node itself. The server offers a `memory` mode that disables the WAL and uses `splice` to copy data; it is intended to pair with a replication algorithm in the future.
...
**rust** reached roughly **860,000 appends/s** at 100k streams, more than an order of magnitude over the reference Node server, which runs out of memory before it reaches that stream count. Group commit lets batches of writes be `fsync`ed together, and WAL sharding lets multiple `fsync` operations run in parallel across the device. ursula runs as a single-node deployment with its WAL disabled, its best case for throughput.

#### Memory usage

Serving a hundred thousand streams, rust's median footprint is about **515 MB**, briefly spiking under a gigabyte during the write burst at initialization. At lower stream counts it sits in the tens to low hundreds of MB. Our Node server is about 800 MB at 10k streams and runs out of memory at 100k.

Once data is appended to the log, reads are always served from disk or the page cache. Read-path memory does not increase with the amount of data being served. The memory growth for rust comes from stream metadata and connection buffers. We are looking into optimizing this next.
...
A thousand clients attach to a single pre-populated stream and replay it from the start — the shape of a fleet reconnecting after a long disconnect. rust peaks at about **2.0 GB/s** in aggregate, with the zero-copy `sendfile` path doing the work; node tops out around **900 MB/s**. ursula keeps pace at the smaller size but does not complete the 2,000-event pre-fill.
...
Durable Streams is the data primitive for serverless agents. With its growing adoption in open source, we set out to build a production-grade reference implementation. Our implementation reaches roughly 860,000 operations a second on a small machine, with stable memory usage. This is the first release, and there is plenty ahead:

- **Benchmarks:** our early benchmarks focus on the write path. We want to build a comprehensive [benchmark suite](https://github.com/electric-sql/ds-bench) that evaluates both read and write paths and stresses stability under load.
- **Multi-node:** a multi-node server for high availability and horizontal scalability.
- **Performance improvements:** we have already identified several optimizations to work on next.
