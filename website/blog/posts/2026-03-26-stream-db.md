...
tags: [agents, durable-streams, agentic, AI, tanstack-db]
outline: [2, 3]
...
[StreamDB](https://durable-streams.com/stream-db) turns a [Durable&nbsp;Stream](/streams/) into a reactive database.
...
<div class="embed-container top" style="padding-bottom: 75%">
  <YoutubeEmbed video-id="ecyLtWlFM2c" title="StreamDB - reactive database in a Durable Stream." />
</div>
...
StreamDB routes events by type into [TanStack&nbsp;DB](/sync/tanstack-db) collections, with end-to-end type-safety from producer to consumer.
...
The underlying [Durable&nbsp;Stream](/streams/) is persistent and addressable. State survives disconnects, refreshes, restarts. Clients resume from their last offset. There's no re-fetching, no state loss.
...
const messagesCollection = createLiveQueryCollection({
  query: (q) => {
    // Group chunks by messageId to derive message metadata.
    ...
    // Use `toArray` with a correlated subquery to collect the
    // ordered chunks for each message group.
    ...
    // Materialize ordered chunks into messages.
    return q
