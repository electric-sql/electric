...
function transformBlogPostPath(pathname: string): string | null {
  ...
  const blogPostMatch = pathname.match(/^\/blog\/(\d{4})\/(\d{2})\/(\d{2})\/([^\/]+?)(\.md)?$/);
...
export default async (request: Request, context: Context) => {
  ...
  const isBlogPost = url.pathname.startsWith('/blog/') &&
  ...
  if (isBlogPost) {
    ...
    if (mdPath) {
      ...
      const shouldServeMd = isExplicitMdRequest || !acceptHeader.includes('text/html');
  ...
  const matchesFullPath = FULL_PATHS.some(path =>
