...
This is a collaborative note-taking app with real-time sync powered by Electric and [Yjs](/docs/sync/integrations/yjs).
...
Notes demonstrates our [Yjs integration](/docs/sync/integrations/yjs). By combining Yjs's powerful collaborative editing capabilities with Postgres and Electric, you get the best of both worlds:
...
[`y-electric.ts`](https://github.com/KyleAMathews/electric-notes/blob/main/src/y-electric/index.ts) provides a standard Yjs provider similar to y-websocket. Internally it syncs Yjs operations using the standard [`ShapeStream`](/docs/sync/api/clients/typescript#shapestream) class from the Electric [Typescript client](/docs/sync/api/clients/typescript).
