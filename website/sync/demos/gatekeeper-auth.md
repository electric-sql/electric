...
This example demonstrates a number of ways of implementing the [gatekeeper auth](/docs/sync/guides/auth#gatekeeper-auth) pattern for [securing access](/docs/sync/guides/auth) to the [Electric sync service](/sync).
...
The auth token should include a claim containing the shape definition. This allows the proxy to authorize the shape request by comparing the shape claim signed into the token with the [shape defined in the request parameters](/docs/sync/quickstart#http-api).
