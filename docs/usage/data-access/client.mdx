...
ElectricSQL provides a schema-aware, type-safe database client from the [electrified subset](../data-modelling/electrification.mdx) of your [Postgres&nbsp;DDL&nbsp;schema](../data-modelling/migrations.mdx).

Generate this Typescript client as part of your build process, instantiate it within your local application and then use it to [sync shapes](./shapes.mdx), [query](./queries.mdx) and [write](./writes.mdx) data.

## Generating the client

Use the [generator script](../../api/generator.mdx) to generate the [Typescript client](../installation/client.mdx):
...
3. configure your [authentication token](../auth/index.mdx) (`config.auth.token`)
4. pass these to your driver's `electrify` function to instantiate the client

For example, for [wa-sqlite](../../integrations/drivers/web/wa-sqlite.mdx) in the web browser:
...
See the next pages on [shapes](./shapes.mdx), [queries](./queries.mdx) and [writes](./writes.mdx) and the <DocPageLink path="api/clients/typescript" /> docs for more detailed information on the specific function calls.
