...
The Electric sync service is an [Elixir application](https://elixir-lang.org) that manages active-active replication between your [Postgres database](./postgres.mdx) and your local apps.
...
You can deploy the sync service together with [a Postgres database](./postgres.mdx) in a Docker Compose file. For example:
