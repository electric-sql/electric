...
ElectricSQL is designed to support applications that sync data between a central cloud database in Postgres and embedded local databases using SQLite. Sync is managed using [Shapes](../data-access/shapes.mdx). Shapes define a **query** that filters the rows in a table against a **segmentation index**.
...
If you can't naturally segment your data then you may find it difficult to optimise the way data syncs onto the local device. You can filter information on a more granular, row-by-row basis using [Permissions](./permissions.mdx) but without clean boundaries it's sometimes harder to make the ElectricSQL paradigm work efficiently.
...
You can create pre-defined segmentation indexes using the [`segment`](../../api/ddlx.mdx#segment) DDLX function with the `type` parameter.
...
If the built-in and pre-defined segmentation indexes are not sufficient, you can define arbitrary segmentation logic using user-defined functions. These are passed by name to the [`segment`](../../api/ddlx.mdx#segment) function using the `get_values_from_row` and `get_segmentation_query` parameters:
