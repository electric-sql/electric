...
If you don't have a data model you can create one using your preferred migrations tooling. You then use the same migrations tooling to extend your data model with [DDLX statements](./electrification.mdx) to expose data to the replication machinery.
...
Alternatively, if you need to create a data model, you can do so using SQL statements like `CREATE TABLE`, or a migrations tool like [Prisma](../../integrations/backend/prisma.mdx) or [Ecto](../../integrations/backend/phoenix.mdx).
...
Use [your prefered migrations framework](../../integrations/backend/index.mdx) to execute [DDLX statements](./electrification.mdx). For example:

<Tabs groupId="migration-framework">
  <TabItem value="ecto" label="Ecto">

With [Phoenix/Ecto](../../integrations/backend/phoenix.mdx) you can use the [`execute/1`](https://hexdocs.pm/ecto_sql/Ecto.Migration.html#execute/1) function.
...
With [Laravel](../../integrations/backend/laravel.mdx) you can use the [`statement` method on the `DB` facade](https://laravel.com/docs/10.x#databases-and-migrations).
...
With [Prisma](../../integrations/backend/prisma.mdx) you [customize a migration to include an unsupported feature](https://www.prisma.io/docs/guides/migrate/developing-with-prisma-migrate/include-unsupported-database-features).
