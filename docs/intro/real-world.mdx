...
So far, we've seen how local-first provides [instant reactivity](./local-first.mdx) and [realtime sync](./multi-user.mdx). We've looked under the hood at [conflict-free offline](./offline.mdx), [active-active replication](./active-active.mdx) and [dynamic sync controls](./sync-controls.mdx).
...
In this architecture, ElectricSQL replaces traditional APIs and state transfer protocols, like REST and GraphQL. You still define your data model [using your existing web framework](../usage/data-modelling/migrations.mdx), like Prisma, Phoenix or Rails -- any framework that handles migrations and works with Postgres.
...
To dive in and start coding, see the [Quickstart](../quickstart/index.mdx).

For more detailed information, see the [Usage](../top-level-listings/usage.mdx) and [Integrations](../top-level-listings/integrations.mdx) guides. If you'd like to chat anything through, join the [community Discord](https://discord.electric-sql.com) and feel free to ask any questions there.
