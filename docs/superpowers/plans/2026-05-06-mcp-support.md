...
## Known gap: cross-process registry refresh after OAuth

Surfaced during integration testing against Honeycomb's MCP server (`mcp.honeycomb.io`).

**The problem:** when the agents-server's OAuth callback persists tokens to the shared vault, the agents process's in-memory registry doesn't notice. Its `applyConfig` only runs on `mcp.json` file changes, so a server that was registered as `needs_auth` (no token) at startup keeps its empty `tools` cache even after the user authorizes — until something independently triggers `applyConfig` again. From the agent's perspective, only the four synthetic bridges (`list_resources` / `read_resource` / `list_prompts` / `get_prompt`) appear; the real MCP-server tools never get fetched via `client.listTools()`.

The agents-server side already handles this via the `OAuthRouteDeps.onAuthorized` hook, which calls `mcpStatusRegistry.refresh()` immediately after a callback persists tokens. The **agents process** has no equivalent.

**Workarounds today:** touch `mcp.json` (rewrite the same content) to fire the file-watcher, which re-applies config and lets the registry re-resolve `needs_auth` servers. Or restart the agents process. Both are hacks.

**Proper fix (follow-up task):** add a vault-change subscription so the agents process's registry knows when to retry connect for `needs_auth` servers. Three viable shapes:

1. **Push from agents-server** — agents-server exposes `POST /api/mcp/internal/server-authorized` (or fires a Durable Streams event). The agents process subscribes and re-resolves the named server on receipt. Cleanest if the two processes can talk; matches the existing pattern where agents-server pushes wakes to agents.
2. **Vault file-watch in agents process** — `fs.watch(vaultPath)` triggers a registry pass that re-resolves any `needs_auth` server. Simple but couples the registry to the vault's storage representation, which breaks if a non-file `KeyVault` is plugged in.
3. **Periodic re-resolve loop** — every N seconds, the registry calls `resolveStatus` for `needs_auth` servers and connects newly-healthy ones. Easiest to ship; trades latency (up to N) for simplicity.

> **Update — Phase B supersedes this gap.** The direct-call refactor (below) eliminates the cross-process gap entirely: agents-server stops handling OAuth callbacks, the runtime owns all MCP state, and there is no second registry to keep in sync. The vault-subscription work is no longer needed once Phase B lands.

---

## Phase B — Direct-call refactor (PR-B, experimental)

**Goal:** rebuild the architecture so the runtime owns 100% of MCP state and the UI talks to it directly. Phases 1–4 above shipped a working but proxy-mediated implementation; Phase B trades the proxy and our hand-rolled OAuth/vault for the SDK's `OAuthClientProvider` + a `CredentialStore` seam, and exposes the runtime's HTTP surface for direct UI consumption.

**Spec alignment:** [`docs/superpowers/specs/2026-05-05-mcp-support-design.md`](../specs/2026-05-05-mcp-support-design.md). Feature is **experimental** while in this phase — public surfaces may change without a deprecation cycle.

**End state:**

- agents-server holds zero MCP state. No `oauth-routes.ts`, no `mcp-status-routes.ts`, no proxy handlers. Only `/api/runtimes` discovery.
- The runtime-hosting process (`packages/agents`) constructs a `CredentialStore` at bootstrap and mounts `mountMcpHttp({ registry, credentials, publicUrl })` to expose `/api/mcp/*` and `/oauth/callback/:server`.
- `KeyVault` and our hand-rolled OAuth (DCR, discovery, PKCE, refresh) are gone, replaced by the SDK's `OAuthClientProvider` backed by the `CredentialStore`.
- UI calls `${agentsServerUrl}/api/runtimes` once, then talks to each runtime's public URL directly. Polling for live updates; SSE deferred.
- `addServer` returns `{ state: 'ready' | 'authenticating' | 'error', authUrl?, ... }`; idempotent on unchanged config; per-server `oauthProvider` escape hatch.

### Task B1: Define `CredentialStore` and built-in implementations

**Files:**

- New: `packages/agents-mcp/src/credentials/types.ts` — `CredentialStore` interface (all methods optional)
- New: `packages/agents-mcp/src/credentials/in-memory.ts` — `inMemoryCredentialStore()`
- New: `packages/agents-mcp/src/credentials/env.ts` — `envCredentialStore()` reading `MCP_<SERVER>_API_KEY|CLIENT_ID|CLIENT_SECRET`
- New: `packages/agents-mcp/src/credentials/file.ts` — `fileCredentialStore(path)`, `chmod 0600`, AES-256-GCM when keychain key available
- New: `packages/agents-mcp/src/credentials/os-keychain.ts` — `osKeychainCredentialStore()` via lazy-required `keytar`; silent skip when native dep missing
- New: `packages/agents-mcp/src/credentials/composed.ts` — `composedCredentialStore(...stores)` reads first non-undefined; writes to first store implementing the relevant `save*`
- New: tests for each, plus a composed-priority test asserting env > keychain > file
- Update: `packages/agents-mcp/src/index.ts` — re-export the new constructors

- [ ] B1.1 — Write the interface and all five implementations test-first (red → green per implementation).
- [ ] B1.2 — Composed store: writes that target a method none of the children implement must throw a clear "no writable store available" error, not silently drop.
- [ ] B1.3 — `keytar` is a peer/optional dep; the package must build and test cleanly without it.

### Task B2: Adopt SDK's `OAuthClientProvider`, retire hand-rolled OAuth

**Files:**

- New: `packages/agents-mcp/src/auth/sdk-provider.ts` — adapter that implements `OAuthClientProvider` (from `@modelcontextprotocol/sdk`) and delegates persistence to a `CredentialStore`
- Delete (after callsites migrate): `src/auth/coordinator.ts`, `src/auth/dcr.ts`, `src/auth/discovery.ts`, `src/auth/authorization-code.ts`, `src/auth/client-credentials.ts`, `src/auth/device-code.ts`, `src/auth/api-key.ts` (header injection moves to the transport layer or a tiny inline helper)
- Update: `src/transports/http.ts` — pass the SDK provider to the transport constructor; let the SDK handle 401-retry and refresh

- [ ] B2.1 — Build the adapter against the SDK's interface; unit tests covering tokens load/save, client-info load/save, redirect URL construction.
- [ ] B2.2 — Migrate `apiKey` mode to a tiny header-injection adapter (no need for the full SDK provider).
- [ ] B2.3 — Migrate one OAuth server (Honeycomb) end-to-end on the new path; assert tokens persist across restart via the file store.
- [ ] B2.4 — Delete the old auth/\* modules and their tests once Honeycomb passes; update `index.ts`.

### Task B3: Retire `KeyVault`, route everything through `CredentialStore`

**Files:**

- Delete: `packages/agents-mcp/src/vault/*`
- Update: `src/registry.ts`, `src/transports/http.ts`, all callsites — `KeyVault` references become `CredentialStore`
- Update: `packages/agents/src/bootstrap.ts` — drop `createFileVault` wiring; construct the default `composedCredentialStore(env, osKeychain, file)` instead

- [ ] B3.1 — Type-driven refactor: rename, fix the compile errors, update tests.
- [ ] B3.2 — Default store wiring at bootstrap matches the spec; log which stores are active at startup.

### Task B4: Refactor registry — connection state machine + idempotency + `oauthProvider` escape hatch

**Files:**

- Update: `packages/agents-mcp/src/registry.ts` — `addServer` returns `AddServerResult`; idempotency check on `(name, url, transport, authMode, scopes, command, args)`; honors per-server `oauthProvider` field bypassing the default `CredentialStore` adapter.
- Update: `src/types.ts` — add `AddServerResult` discriminated union; widen `McpAuthMode` config to permit `oauthProvider` (runtime-only) or `oauthProviderRef` (config-file).
- New tests covering each return state + the idempotency no-op.

- [ ] B4.1 — Idempotency test: `applyConfig` called twice in a row with the same config does not close transports or reissue listTools.
- [ ] B4.2 — `addServer` returns `authenticating` with a populated `authUrl` for an OAuth server with no token; transitions to `ready` once OAuth completes.
- [ ] B4.3 — `oauthProvider` escape hatch: passing a custom provider bypasses the `CredentialStore` for that one server only.

### Task B5: `mountMcpHttp` — runtime-side HTTP surface

**Files:**

- New: `packages/agents-mcp/src/http/mount.ts` — exports `mountMcpHttp({ registry, credentials, publicUrl, app, corsOrigin })`
- Routes mounted: `GET /api/mcp/servers`, `POST /api/mcp/servers`, `POST /api/mcp/servers/:name/authorize|disable|enable|reconnect`, `DELETE /api/mcp/servers/:name`, `GET /oauth/callback/:server`, `POST /oauth/device/:server/start`
- New tests asserting each route's contract against a fake registry

- [ ] B5.1 — All routes return the same `AddServerResult` envelope where applicable.
- [ ] B5.2 — `/oauth/callback/:server` calls `provider.finishAuth(code)`, persists via `CredentialStore`, renders the success page.
- [ ] B5.3 — CORS allowlist accepts the agents-server origin (configurable; `*` default in dev only).
- [ ] B5.4 — Reserve a `requireAuth?: (req) => boolean` hook on `mountMcpHttp` for the future bearer-token check (no implementation yet).

### Task B6: agents-server discovery endpoint + extended type registration

**Files:**

- Update: `packages/agents-server/src/server.ts` — extend the `POST /_electric/agents/types` handler to accept `publicUrl`; store `(name, publicUrl, types[])` in memory keyed by runtime name
- New: `packages/agents-server/src/runtime-registry.ts` (or inline in `server.ts`) — small in-memory map; cleared on restart
- New route: `GET /api/runtimes` returning the registered runtimes
- Update: `packages/agents-runtime/src/create-handler.ts` — include `publicUrl` (sourced from `publicUrl` option passed by the embedder) in the registration payload

- [ ] B6.1 — Backward-compat: registrations without `publicUrl` are accepted but the runtime is omitted from `/api/runtimes` (logged once).
- [ ] B6.2 — Test that re-registration on runtime restart replaces the prior entry.

### Task B7: Delete the proxy

**Files:**

- Delete: `packages/agents-server/src/oauth-routes.ts`, `packages/agents-server/src/mcp-status-routes.ts`, `packages/agents-server/src/mcp-proxy.ts` (and any related fixtures)
- Update: `packages/agents-server/src/server.ts` — remove the mounts and the `mcpStatusRegistry` plumbing
- Update: `packages/agents/src/bootstrap.ts` — drop the `onAuthorized → POST /_electric/mcp/refresh` hook and the periodic backstop

- [ ] B7.1 — `pnpm -r typecheck` clean after deletions.
- [ ] B7.2 — Manual: hit a removed agents-server `/api/mcp/servers` URL → 404.

### Task B8: UI — talk to runtimes directly

**Files:**

- Update: `packages/agents-server-ui/src/hooks/useRuntimes.ts` (new) — fetches `/api/runtimes` once, refetches on focus + every 60s
- Update: `packages/agents-server-ui/src/components/connected-services/*.tsx` — for each runtime in the discovery result, fetch `${runtime.publicUrl}/api/mcp/servers`; merge and render keyed by `${runtimeName}/${serverName}`
- Update mutations (Authorize / Disable / Reconnect / Disconnect) to call the runtime directly
- Polling: idle = 10s; active OAuth (`status === 'authenticating'`) = 2s

- [ ] B8.1 — CORS works: the agents-server origin can hit `http://localhost:4448/api/mcp/servers` from the browser.
- [ ] B8.2 — Multi-runtime smoke: register two runtimes, verify both appear in the catalog with their respective servers.
- [ ] B8.3 — End-to-end OAuth from the UI: click "Authorize honeycomb" → new tab → consent → status flips to `ready` within one poll interval.

### Task B9: Mark experimental + docs

**Files:**

- Update: `packages/agents-mcp/src/index.ts` — runtime warning logged once on import: `[agents-mcp] EXPERIMENTAL — public surfaces may change`
- Update: `packages/agents-mcp/README.md` (if present) and the spec — banner on every public-surface section
- Update: `packages/agents-server/src/server.ts` — `/api/runtimes` response includes `experimental: true` flag so consumers can branch

- [ ] B9.1 — Console warning fires once per process, not per import.
- [ ] B9.2 — Type-level `@experimental` JSDoc on every exported symbol introduced by Phase B.

### Task B10: Phase B verification

- [ ] B10.1 — `pnpm -r typecheck && pnpm -r test --run` green.
- [ ] B10.2 — End-to-end: fresh checkout → start agents-server + builtin-agents → UI loads, discovers runtime, registers Honeycomb via `mcp.json`, completes OAuth, tools list populates, an agent invokes a tool successfully.
- [ ] B10.3 — Restart only the runtime: re-registration repopulates `/api/runtimes`; UI recovers without reload.
- [ ] B10.4 — Restart only agents-server: runtimes re-announce on next handshake; UI reconnects.
- [ ] B10.5 — Manual sweep of files-deleted-by-this-phase: nothing imports them; no dead branches in `server.ts`.
- [ ] B10.6 — Confirm "known gap: cross-process registry refresh" no longer applies; remove the section above (or replace with a one-line "resolved by Phase B" note).

---

### Out of scope for Phase B (tracked for later)

- **Stream-based MCP registration** via the `entity_types` shape pattern. Considered, deferred — direct calls ship first; we revisit once we have multiple runtimes + multi-tab UI in real use.
...
- **SSE on `/api/mcp/events`.** Polling is fine until it isn't; this is the next iteration if/when polling becomes the bottleneck.
- **Tunnel the OAuth callback through agents-server** (so the runtime doesn't need a public URL). Possible alternative deployment shape; out of scope here.

Recommend **option 1** as the long-term shape and **option 3** as a stop-gap (e.g. 30 s interval). The plan should add a task: `Phase 3.5: Cross-process registry refresh after OAuth`. Until that lands, document the workaround prominently in the OAuth section of the spec.
