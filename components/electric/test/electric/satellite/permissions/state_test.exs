defmodule Electric.Satellite.Permissions.StateTest do
  ...
  def update_global(mock_loader, ddlx) do
    {:ok, rules} = SchemaLoader.global_permissions(mock_loader)

    {:ok, _, rules} = State.apply_ddlx(ddlx, rules)

    {:ok, loader} = MockSchemaLoader.save_global_permissions(mock_loader, rules)
  ...
  def loader_with_global_perms(cxt, ddlx \\ default_ddlx()) do
    ...
    {loader, rules} = update_global(loader, ddlx)
