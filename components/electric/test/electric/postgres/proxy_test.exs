defmodule Electric.Postgres.ProxyTest do
  ...
  test "electrified index tracking", cxt do
    ...
    assert {:ok, true} = Extension.electrified?(cxt.conn, "public", "orchid")
    ...
    assert {:ok, true} = Extension.index_electrified?(cxt.conn, "public", "orchid_id_idx")
    ...
    query = "DROP INDEX public.orchid_id_idx"
    ...
    assert {:ok, false} = Extension.index_electrified?(cxt.conn, "public", "orchid_id_idx")
  end
