defmodule Electric.Satellite.Permissions do
  ...
  defmodule MoveOut do
    ...
    @type t() :: %__MODULE__{change: Changes.change()}
  end
  ...
  defstruct [
    ...
    :grants,
    :structure,
    source: %{rules: %{grants: [], assigns: []}, roles: [], schema: nil, id: 0},
  ...
  def update!(%__MODULE__{} = perms, %SchemaLoader.Version{} = schema, rules, roles) do
    case update(perms, schema, rules, roles) do
      ...
      {:error, perms, reason} ->
        raise Error, message: reason, perms: perms
    end
  end

  def update!(%__MODULE__{} = perms, %SchemaLoader.Version{} = schema, %SatPerms{} = sat_perms) do
    case update(perms, schema, sat_perms) do
      {:ok, perms} ->
        perms
  ...
  """
  @spec update(empty() | t(), SchemaLoader.Version.t(), %SatPerms{}) ::
          {:ok, t()} | {:error, source(), String.t()}
  def update(%__MODULE__{} = perms, %SchemaLoader.Version{} = schema, %SatPerms{} = sat_perms) do
    update(perms,
      schema: schema,
      rules: sat_perms.rules,
      roles: sat_perms.roles,
      id: sat_perms.id
    )
  end
  ...
  def update(%__MODULE__{} = perms, attrs) when is_list(attrs) do
    ...
    |> update_perms(Keyword.get(attrs, :perms))
    |> update_id(Keyword.get(attrs, :id))
    |> rebuild()
  end
  ...
  defp update_roles(perms, %SatPerms.Roles{roles: roles}) when is_list(roles) do
    update_roles(perms, roles)
  end
  ...
  defp update_perms(perms, nil) do
    perms
  end

  defp update_perms(perms, %SatPerms{} = sat_perms) do
    perms
    |> update_roles(sat_perms.roles)
    |> update_rules(sat_perms.rules)
    |> update_id(sat_perms.id)
  end

  defp update_id(perms, nil) do
    perms
  end

  defp update_id(perms, id) when is_integer(id) do
    %{perms | source: %{perms.source | id: id}}
  end
  ...
  defp rebuild(perms) do
    ...
    case Structure.compile(rules.assigns, rules.grants, schema_version) do
      {:ok, structure} ->
        {:ok,
         %{
           perms
  ...
  def receive_transaction(perms, %Changes.Transaction{} = tx) do
    Map.update!(
      perms,
      :write_buffer,
      &WriteBuffer.receive_transaction(&1, perms.structure, tx)
    )
  end

  def build_role_grants(roles, grants) do
    roles
    |> Enum.map(&{&1, Role.matching_grants(&1, grants)})
    |> Enum.reject(fn {_role, grants} -> Enum.empty?(grants) end)
    |> Enum.flat_map(&invert_role_lookup/1)
  ...
  defp add_authenticated(roles, nil) do
    roles
  end
  ...
  @doc """
  Return a list of permissions test results and their corresponding `RoleGrant` structs for a
  given change.

  Implemented like this because the read filtering needs to post-process any changes that fail
  the permissions check in order to detect where-clause failures after an update, so we need
  to pass the list of possible grants in order to detect the changing row status.
  """
  @spec read_permissions(change(), t(), Graph.impl(), xid()) :: [{boolean(), RoleGrant.t() | nil}]
  # never filter out electric metadata tables
  def read_permissions(%{relation: relation}, _perms, _graph, _xid)
      when is_extension_relation(relation) do
    [{true, nil}]
  end

  def read_permissions(change, perms, graph, xid) do
    change
    |> role_grants_for_read(perms)
    |> role_grant_for_change(perms, graph, change, xid, :read)
  end

  defp role_grants_for_read(change, perms) do
    Map.get(perms.roles, {change.relation, :SELECT}, [])
  end

  @spec id(t(), change()) :: [id()]
  def id(perms, change) do
    Structure.pk_val(perms.structure, change)
  end
  ...
  def validate_write(%__MODULE__{} = perms, graph, %Changes.Transaction{} = tx) do
    ...
    |> validate_all_writes(perms, graph, tx.xid)
  end
  ...
  defp modifies_scope_fk?(change, %{structure: structure} = perms, _graph) do
    Enum.any?(perms.scopes, &Structure.modified_fks?(structure, &1, change))
  end

  defp validate_all_writes(changes, perms, graph, xid) do
    with {:ok, write_buffer} <- validate_writes_with_scope(changes, perms, graph, xid) do
      {:ok, %{perms | write_buffer: write_buffer}}
    end
  end

  defp validate_writes_with_scope(changes, perms, graph, xid) do
      ...
      fn change, {:ok, write_buffer} ->
        case verify_write(change, perms, write_buffer, xid) do
          ...
          %{role: role, grant: grant} = _role_grant ->
            ...
            write_buffer =
              write_buffer
              |> Graph.apply_change(perms.structure, change)
  ...
  defp verify_write(change, perms, graph, xid) do
    ...
    role_grant =
      ...
      |> role_grant_for_change(perms, graph, change, xid, :write)
  ...
  @spec role_grant_for_change([], t(), Graph.impl(), change(), xid(), mode()) :: []
  @spec role_grant_for_change(nil, t(), Graph.impl(), change(), xid(), mode()) :: nil
  defp role_grant_for_change(empty, _perms, _scope, _change, _xid, _mode)
       when empty in [nil, []] do
    empty
  end

  @spec role_grant_for_change(assigned_roles(), t(), Graph.impl(), change(), xid(), mode()) ::
          RoleGrant.t() | nil
  defp role_grant_for_change(grants, perms, graph, change, xid, mode) do
    %{unscoped: unscoped_role_grants, scoped: scoped_role_grants} = grants

    Enum.concat([
      unscoped_role_grants,
      scoped_role_grants(scoped_role_grants, perms, graph, change, xid),
      transient_role_grants(scoped_role_grants, perms, graph, change, xid)
    ])
    |> find_grant_allowing_change(change, mode)
  end

  defp scoped_role_grants(role_grants, perms, graph, change, _xid) do
    Stream.filter(role_grants, fn
      %{role: %{scope: {scope_table, scope_id}}} ->
        ...
        change_in_scope?(graph, perms, scope_table, scope_id, change)
    end)
  end

  defp transient_role_grants(role_grants, perms, graph, change, xid) do
    role_grants
    |> Transient.for_roles(xid, perms.transient_lut)
    |> Stream.flat_map(fn {role_grant, %Transient{target_relation: relation, target_id: id} = tdp} ->
      if change_in_scope?(graph, perms, relation, id, change) do
        Logger.debug(fn ->
  ...
  @spec find_grant_allowing_change([RoleGrant.t()], change(), :read) ::
          {boolean(), change(), RoleGrant.t()}
  defp find_grant_allowing_change(role_grants, change, :read) do
    Enum.map(
      role_grants,
      fn %{grant: grant} = role_grant ->
        {change_passes_check?(grant, change), role_grant}
      end
    )
  end
  ...
  defp change_in_scope?(graph, perms, scope_relation, scope_id, change) do
    graph
    |> Graph.scope_id(perms.structure, scope_relation, change)
