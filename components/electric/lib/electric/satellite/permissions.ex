defmodule Electric.Satellite.Permissions do
  ...
  def new(%Auth{} = auth, {_, _} = graph, transient_lut_name \\ Transient) do
    %__MODULE__{
      auth: auth,
      graph: Graph.new(read: graph, write: WriteBuffer.new(graph, auth)),
  ...
  def receive_transaction(perms, %Changes.Transaction{} = tx) do
    %{graph: %{write: write_buffer} = graph} = perms
    write_buffer = WriteBuffer.receive_transaction(write_buffer, tx)
    %{perms | graph: %{graph | write: write_buffer}}
  end
