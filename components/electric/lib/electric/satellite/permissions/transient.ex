defmodule Electric.Satellite.Permissions.Transient do
  ...
  @type t() :: %__MODULE__{
          ...
          target_id: binary(),
          valid_to: Electric.Postgres.xid()
        }
  ...
  a valid grant.
  """
  @spec for_roles([RoleGrant.t()], Permissions.xid(), lut()) :: Enum.t()
  def for_roles(role_grants, xid, table) do
    role_grants
    |> Enum.flat_map(&transient_for_roles(&1, table))
    |> Stream.filter(&filter_expired(&1, xid))
  end
  ...
  defp filter_expired({_role_grant, %__MODULE__{valid_to: expires_xid}}, change_xid) do
    expires_xid >= change_xid
  end
