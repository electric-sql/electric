defmodule Electric.Satellite.ClientReconnectionInfo do
  ...
  def store_initial_checkpoint(client_id, lsn, sent_rows_graph) do
    ...
    :ets.insert(@checkpoint_ets, {client_id, lsn, nil, sent_rows_graph})
  end
  ...
  def advance_checkpoint(client_id, opts) do
    case :ets.lookup(@checkpoint_ets, client_id) do
      ...
      [{_, acked_lsn, _acked_txid, graph}] ->
        {new_lsn, new_txid} = Keyword.fetch!(opts, :ack_point)
        ...
        if CachedWal.Api.compare_positions(cached_wal_impl, acked_lsn, new_lsn) != :gt do
          ...
          :ets.insert(@checkpoint_ets, {client_id, new_lsn, new_txid, new_graph})
  ...
  defp advance_up_to_new_lsn(
    ...
    # {{client_id, transaction_id}, actions}
  ...
  def list_subscriptions(client_id) do
    :ets.select(@subscriptions_ets, [
      {{{client_id, :_}, :"$1", :"$2", :_}, [], [{{:"$1", :"$2"}}]}
    ])
  end

  def list_subscriptions(client_id, xmin_before) do
    :ets.select(
      @subscriptions_ets,
      [
        {{{client_id, :_}, :"$1", :"$2", :_}, [{:<, :"$1", xmin_before}], [{{:"$1", :"$2"}}]}
      ]
    )
  end
