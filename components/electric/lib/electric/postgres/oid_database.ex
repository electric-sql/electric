defmodule Electric.Postgres.OidDatabase do
  # `ETS.Set.match!/2` has incorrect spec
  @dialyzer {:nowarn_function, oid_for_name: 1}
  import Electric.Postgres.OidDatabase.PgType

  use GenServer

  @ets_table_name :oid_database
  def start_link(opts) do
    GenServer.start_link(__MODULE__, [], Keyword.take(opts, [:name]))
  end

  def save_oids(server \\ __MODULE__, values) do
    GenServer.call(server, {:save, Enum.map(values, &pg_type_from_tuple/1)})
  end

  @doc """
  Get an atom name by the type OID
  """
  def name_for_oid(oid) do
    table = ETS.Set.wrap_existing!(@ets_table_name)

    case ETS.Set.get!(table, oid) do
      pg_type(is_array: false, name: name) -> name
      pg_type(is_array: true, element_oid: element) -> {:array, name_for_oid(element)}
      _ -> raise("Unknown OID #{oid}, cannot get a name that won't result in data loss")
    end
  end

  @doc """
  Get the type OID by the name atom
  """
  def oid_for_name({:array, element_name}) do
    table = ETS.Set.wrap_existing!(@ets_table_name)

    case ETS.Set.match!(table, pg_type(name: element_name, array_oid: :"$1")) do
      [oid] -> List.first(oid)
      _ -> raise("Unknown name {:array, #{element_name}}")
    end
  end

  def oid_for_name(name) do
    table = ETS.Set.wrap_existing!(@ets_table_name)

    case ETS.Set.match!(table, pg_type(name: name, oid: :"$1")) do
      [[oid]] -> oid
      _ -> raise("Unknown name #{name}")
    end
  end

  def type_length(oid) when is_integer(oid) do
    pg_type(length: len) = ETS.Set.get!(ETS.Set.wrap_existing!(@ets_table_name), oid)
    len
  end

  def type_length(name) do
    [pg_type(length: len)] =
      ETS.Set.match_object!(ETS.Set.wrap_existing!(@ets_table_name), pg_type(name: name))

    len
  end

  def init(_) do
    {:ok, table} =
      ETS.Set.new(
        name: @ets_table_name,
        keypos: 2,
        protection: :protected,
        read_concurrency: true
      )

    Electric.Postgres.OidDatabase.Defaults.write_defaults(table)

    {:ok, %{table: table}}
  end

  def handle_call({:save, values}, _, state) do
    ETS.Set.put(state.table, values)

    {:reply, :ok, state}
  end
end
