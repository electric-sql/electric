...
export async function loadDockerode(): Promise<DockerCtor> {
  try {
    // `@vite-ignore` keeps Vite from resolving/bundling this optional
    // runtime-only dep at build time (callers externalize it).
    const mod = (await import(/* @vite-ignore */ `dockerode`)) as unknown as {
      default?: DockerCtor
    }
...
export async function isDockerAvailable(): Promise<boolean> {
  ...
  try {
    ...
    // Surface why docker is unavailable (missing dockerode vs. daemon down).
    console.warn(
