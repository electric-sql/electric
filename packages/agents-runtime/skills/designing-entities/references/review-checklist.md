...
| S4  | `inboxSchemas` are defined if the handler branches on `wake.type === "inbox"` with typed payloads per message type.                         | Schemas surface in the Electric Agents UI/CLI and validate incoming messages. Untyped inbox = silent data shape drift.                     |
...
| M4  | `ctx.spawn(...)` that needs a child result uses `wake: { on: "runFinished", includeResponse: true }`, records child metadata in state, returns from the current wake, and continues from the later child-completion wake.                                                                                                                                  | Same-wake child awaits are not durable orchestration. The runFinished wake is the continuation mechanism.                                                                                  |
| M5  | `ctx.observe(entity(url), ...)` includes a `wake` option (`{ on: "change", collections: [...] }` or `{ on: "runFinished", includeResponse: true }`) whenever the handler needs to react to the observed entity. `observe()` takes an `ObservationSource` (use `entity()`, `cron()`, or `entities()` from `@electric-ax/agents-runtime`), not a raw string. | Observation without wake is a silent subscription that never re-invokes the handler. Raw strings are not valid `ObservationSource` objects.                                                |
...
4. **Missing `wake: { on: "runFinished", includeResponse: true }` on result-producing spawn.** Parent never receives the child-completion continuation with response text.
...
Universal checks:
  ...
  ✗ M4 runFinished continuation wake missing on result-producing spawn
  N/A S3 creationSchema — no spawn args expected

Pattern-specific (<name>): see patterns/<name>.md

Proposed fixes:
  - M4: add `wake: { on: "runFinished", includeResponse: true }` to ctx.spawn() options and return; continue from the later wake
