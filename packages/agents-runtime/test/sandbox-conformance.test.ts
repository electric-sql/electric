...
interface ProviderFactory {
  ...
  /**
   * Whether READS are contained to the workspace. Writes are contained on
   * every provider; reads are contained on unrestricted/docker but `remote`
   * allows reads anywhere in the VM (see Sandbox FS-method contract).
   */
  containsReads: boolean
}
...
> = [
  ...
  },
  {
    ...
    enabled: true,
    capabilities: {
      // The in-memory fake doesn't forward signals.
      supportsAbort: false,
    },
    outsideKind: `etc-passwd`,
    // remote deliberately allows reads outside the workspace (the VM is
    // already host-isolated); only writes are contained.
    containsReads: false,
    create: (cwd) =>
      remoteSandbox({
        provider: `e2b`,
...
describe(`sandbox conformance`, () => {
  ...
  for (const provider of providers) {
    ...
    d(provider.name, () => {
      ...
      it(`writeFile outside the working directory is rejected with a policy SandboxError`, async () => {
        // Workspace containment is a sandbox concern, enforced uniformly
        // across every provider against the filesystem it owns. Unrestricted
        // shares the host FS and contains via realpath; docker/remote contain
        // against their container/VM root.
        const sandbox = await provider.create(cwd)
        const outside =
      ...
      it(`readFile outside the working directory follows the provider's read-containment policy`, async () => {
        const sandbox = await provider.create(cwd)
        const outside =
          provider.outsideKind === `etc-passwd`
            ? `/etc/passwd`
            : join(tmpdir(), `conformance-read-outside-${Date.now()}.txt`)
        if (provider.outsideKind === `host-tempdir`) {
          await writeFile(outside, `host-secret`)
        }
        try {
          if (provider.containsReads) {
            await expect(sandbox.readFile(outside)).rejects.toMatchObject({
              kind: `policy`,
            })
          } else {
            // remote allows reads anywhere in the VM — the read is not
            // policy-gated. (Against the in-memory fake the path is absent,
            // so it surfaces as an ENOENT, not a policy rejection; the
            // invariant under test is the absence of a policy gate.)
            await sandbox.readFile(outside).then(
              () => {},
              (err: unknown) => {
                const isPolicy =
                  err instanceof SandboxError && err.kind === `policy`
  ...
  // where the sandbox follows the link, sees the realpath leave the
  // workspace, and rejects. Skip for remote (VM-rooted, fake doesn't model
  // symlinks) and docker (container fs, host workdir isn't mounted in).
  for (const provider of providers.filter(
    ...
    d(`${provider.name} — symlink escape`, () => {
      ...
      it(`readFile rejects a symlink pointing outside the workspace`, async () => {
        ...
        const sandbox = await provider.create(cwd)
        try {
          // Symlink defense is a sandbox concern: unrestricted resolves the
          // realpath, sees it escape the workspace, and rejects with policy.
          await expect(
