...
describe(`budget enforcement`, () => {
  ...
  it(`stubs oversized tool_result content instead of dropping it`, async () => {
    ...
    expect(
      (toolCalls[0] as { toolCallId?: string } | undefined)?.toolCallId
    ).toBe(`tc-1`)
    expect(
      (toolResults[0] as { toolCallId?: string } | undefined)?.toolCallId
    ).toBe(`tc-1`)
  ...
  it(`drops orphaned tool_results when their tool_call is budget-truncated`, async () => {
    ...
    for (const result of toolResults) {
      const resultId = (result as { toolCallId?: string }).toolCallId
      expect(
        toolCalls.some(
          (call) => (call as { toolCallId?: string }).toolCallId === resultId
        )
      ).toBe(true)
    }
    for (const call of toolCalls) {
      ...
      expect(
        toolResults.some(
          (result) => (result as { toolCallId?: string }).toolCallId === callId
        )
      ).toBe(true)
    }
  })
