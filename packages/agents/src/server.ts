...
export class BuiltinAgentsServer {
  ...
  async start(): Promise<string> {
    ...
    const mcpConfigPath = this.options.loadProjectMcpConfig
      ? path.resolve(this.options.workingDirectory ?? process.cwd(), `mcp.json`)
      : null
    ...
    const wirePersistence = async (cfg: McpConfig): Promise<McpConfig> => {
      ...
      for (const s of cfg.servers) {
        if (s.transport === `http` && s.auth?.mode === `authorizationCode`) {
          ...
          servers.push({ ...s, auth: { ...s.auth, ...persist } })
        } else {
          servers.push(s)
        }
      }
    ...
    const merge = (jsonCfg: McpConfig | null): McpConfig => {
      ...
      return { servers: [...filteredExtras, ...jsonServers], raw: jsonCfg?.raw }
    }
    ...
    if (mcpConfigPath) {
      try {
        void applyMerged(await loadMcpConfig(mcpConfigPath, process.env))
      } catch (err) {
    ...
    registerToolProvider({
      ...
      tools: () => {
        ...
        for (const entry of mcpRegistry.list()) {
          ...
          const caps = (live.transport.client as any).getServerCapabilities?.()
          if (caps?.resources)
            tools.push(
              ...buildResourceTools({
                server: entry.name,
