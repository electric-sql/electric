...
| L2  | `#requestShape` catch → `#requestShape` | 966  | Abort marked for restart by `forceDisconnectAndRefresh` or system wake | `isRefreshing` flag changes `canLongPoll`, affecting `live` param                                                 | Restart intent is tracked on the controller, not solely by `AbortSignal.reason`, because some runtimes (e.g. React Native) do not preserve abort reasons |
