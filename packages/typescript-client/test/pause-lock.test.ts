...
describe(`PauseLock`, () => {
  ...
  it(`warns when lock is held too long`, async () => {
    const warnSpy = vi.spyOn(console, `warn`).mockImplementation(() => {})
    const onAcquired = vi.fn()
    const onReleased = vi.fn()
    ...
    expect(warnSpy).toHaveBeenCalledOnce()
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining(`PauseLock held for 50ms`)
    )

    lock.release(`slow-op`)
    warnSpy.mockRestore()
  })

  it(`does not warn when lock is released before timeout`, async () => {
    const warnSpy = vi.spyOn(console, `warn`).mockImplementation(() => {})
    const onAcquired = vi.fn()
    const onReleased = vi.fn()
    const lock = new PauseLock({
    ...
    lock.acquire(`fast-op`)
    lock.release(`fast-op`)
  ...
  it(`does not warn when warnHeldAfterMs is not set`, async () => {
    const warnSpy = vi.spyOn(console, `warn`).mockImplementation(() => {})
    const { lock } = createLock()
  ...
  it(`clears warning timer on releaseAllMatching`, async () => {
    const warnSpy = vi.spyOn(console, `warn`).mockImplementation(() => {})
    const onAcquired = vi.fn()
    const onReleased = vi.fn()
    const lock = new PauseLock({
