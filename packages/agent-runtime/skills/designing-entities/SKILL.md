...
- Handler outline — `firstWake` init, wake-type branches, agent config, spawn/observe calls
  ...
  - Observation sources: `entity(url)`, `cron(expression)`, `tagged({ match: ... })` — import from `@electric-ax/agent-runtime`
...
```ts
import type { EntityRegistry } from "@electric-ax/agent-runtime"
