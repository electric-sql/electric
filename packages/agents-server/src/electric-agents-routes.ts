...
export class ElectricAgentsRoutes {
  ...
  async handleRequest(
    ...
    if (entityMatch) {
      ...
      if (method === `PATCH`) {
        await this.handlePatchArgs(path, req, res)
        return true
      }
    }
  ...
  private async handlePatchArgs(
    ...
    const entity = await this.manager.registry.getEntity(entityUrl)
    if (!entity) {
      sendJsonError(res, 404, `NOT_FOUND`, `Entity not found at ${entityUrl}`)
      return
    }

    const body = await parseJsonBody<{ args?: unknown }>(req, res)
    if (!body) return
    ...
    if (
      ...
      sendJsonError(
        ...
        400,
        `INVALID_REQUEST`,
        `Body must be { args: Record<string, unknown> }`
      )
      return
    }
    ...
    try {
      const txid = await this.manager.registry.updateEntitySpawnArgs(
        entityUrl,
        merged
      )
      sendJson(res, 200, { txid })
    } catch (err) {
      handleElectricAgentsError(err, res)
    }
  }
