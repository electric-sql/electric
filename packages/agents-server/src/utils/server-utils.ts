...
export function buildElectricProxyTarget(options: {
  ...
  if (table === `entities`) {
    ...
    target.searchParams.set(
      `columns`,
      `"tenant_id","name","description","creation_schema","inbox_schemas","state_schemas","custom_collection_schemas","slash_commands","serve_endpoint","default_dispatch_policy","revision","created_at","updated_at"`
    )
    applyShapeWhere(
      target,
