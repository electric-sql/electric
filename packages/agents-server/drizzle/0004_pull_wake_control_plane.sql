...
CREATE TABLE runners (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT uq_runners_wake_stream UNIQUE (wake_stream),
...
CREATE INDEX idx_runners_owner_user_id
  ON runners (owner_user_id);

CREATE INDEX idx_runners_admin_status
  ON runners (admin_status);

CREATE INDEX idx_runners_liveness_lease_expires_at
  ON runners (liveness_lease_expires_at);

CREATE TABLE entity_dispatch_state (
  entity_url text PRIMARY KEY,
...
CREATE INDEX idx_entity_dispatch_state_active_runner
  ON entity_dispatch_state (active_runner_id);

CREATE INDEX idx_entity_dispatch_state_outstanding_wake
  ON entity_dispatch_state (outstanding_wake_id);

CREATE INDEX idx_entity_dispatch_state_active_lease
  ON entity_dispatch_state (active_lease_expires_at);
