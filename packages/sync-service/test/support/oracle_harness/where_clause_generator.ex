defmodule Support.OracleHarness.WhereClauseGenerator do
  ...
  def generate_where_clauses(count, seed) do
    # Create a deterministic stream using the seed
    seed_state = :rand.seed_s(:exsss, seed)

    where_clause_gen()
    |> Enum.take(count * 3)
    |> Enum.uniq_by(fn {where, _optimized} -> where end)
    |> Enum.take(count)
    |> shuffle_with_seed(seed_state)
    |> Enum.map(fn {where, optimized} ->
      %{where: where, optimized: optimized}
    end)
  end

  defp shuffle_with_seed(list, seed_state) do
    {shuffled, _} =
      Enum.reduce(list, {[], seed_state}, fn item, {acc, state} ->
        {rand_val, new_state} = :rand.uniform_s(state)
        {[{rand_val, item} | acc], new_state}
      end)

    shuffled
    |> Enum.sort_by(fn {rand_val, _} -> rand_val end)
  ...
  defp between_gen do
    bind(
      {member_of([{"BETWEEN", true}, {"NOT BETWEEN", true}]),
       value_literal_gen(),
       value_literal_gen()},
  ...
  defp or_composition(depth) do
    bind({base_expr_gen(depth - 1), base_expr_gen(depth - 1)}, fn
      {{left, left_opt}, {right, right_opt}} ->
        ...
        optimized = left_opt and right_opt and not contains_subquery?(left) and not contains_subquery?(right)
