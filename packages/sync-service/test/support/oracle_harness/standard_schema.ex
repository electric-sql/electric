defmodule Support.OracleHarness.StandardSchema do
  ...
  defp toggle_level_1_active_gen do
    ...
    |> map(fn id ->
      %{name: "toggle_l1_#{id}", sql: "UPDATE level_1 SET active = NOT active WHERE id = '#{id}'", row_key: {:level_1, id}}
    end)
  end
  ...
  defp toggle_level_2_active_gen do
    ...
    |> map(fn id ->
      %{name: "toggle_l2_#{id}", sql: "UPDATE level_2 SET active = NOT active WHERE id = '#{id}'", row_key: {:level_2, id}}
    end)
  end
  ...
  defp toggle_level_3_active_gen do
    ...
    |> map(fn id ->
      %{name: "toggle_l3_#{id}", sql: "UPDATE level_3 SET active = NOT active WHERE id = '#{id}'", row_key: {:level_3, id}}
    end)
  end
