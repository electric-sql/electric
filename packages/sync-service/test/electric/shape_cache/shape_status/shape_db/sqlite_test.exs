defmodule Electric.ShapeCache.ShapeStatus.ShapeDb.SqliteTest do
  ...
  test "add_shape inserts persistent shape data", ctx do
    ...
    {:ok, _hash1} = Sqlite.add_shape(ctx.stack_id, shape1, handle1)

    assert {:ok, [{^handle1, ^shape1}]} = Sqlite.list_shapes(ctx.stack_id)

    shape2 = Shape.new!("items", inspector: @stub_inspector, where: "id = 1")
    handle2 = "handle-2"

    {:ok, _hash2} = Sqlite.add_shape(ctx.stack_id, shape2, handle2)

    assert {:ok, [{^handle1, ^shape1}, {^handle2, ^shape2}]} = Sqlite.list_shapes(ctx.stack_id)
  end
  ...
  test "handle_exists?", ctx do
    ...
    refute Sqlite.handle_exists?(ctx.stack_id, handle1)
    {:ok, _hash1} = Sqlite.add_shape(ctx.stack_id, shape1, handle1)
    assert Sqlite.handle_exists?(ctx.stack_id, handle1)
  end
  ...
  test "shape_handles_for_relations", ctx do
    ...
    handle3 = "handle-3"
    {:ok, _hash3} = Sqlite.add_shape(ctx.stack_id, shape3, handle3)

    assert {:ok, [^handle1, ^handle2]} =
             Sqlite.shape_handles_for_relations(ctx.stack_id, [{1, {"public", "items"}}])

    assert {:ok, [^handle3]} =
             Sqlite.shape_handles_for_relations(ctx.stack_id, [{2, {"public", "other_table"}}])

    assert {:ok, [^handle1, ^handle2, ^handle3]} =
             Sqlite.shape_handles_for_relations(ctx.stack_id, [
               {1, {"public", "items"}},
  ...
  test "remove_shape", ctx do
    ...
    assert {:ok, [{^handle1, ^shape1}, {^handle2, ^shape2}, {^handle3, ^shape3}]} =
             Sqlite.list_shapes(ctx.stack_id)

    :ok = Sqlite.remove_shape(ctx.stack_id, handle1)

    assert {:ok, [{^handle2, ^shape2}, {^handle3, ^shape3}]} = Sqlite.list_shapes(ctx.stack_id)
    ...
    :ok = Sqlite.remove_shape(ctx.stack_id, handle3)

    assert {:ok, [{^handle2, ^shape2}]} = Sqlite.list_shapes(ctx.stack_id)
    ...
    :ok = Sqlite.remove_shape(ctx.stack_id, handle2)
  ...
  test "remove non-existing shape", ctx do
    ...
    handle1 = "handle-1"
    {:ok, _hash1} = Sqlite.add_shape(ctx.stack_id, shape1, handle1)
    assert {:ok, 1} = Sqlite.count_shapes(ctx.stack_id)

    assert {:error, {:enoshape, "no-such-handle"}} =
             Sqlite.remove_shape(ctx.stack_id, "no-such-handle")
  ...
  test "handle_for_shape/2", ctx do
    ...
    assert {:ok, ^handle1} = Sqlite.handle_for_shape(ctx.stack_id, shape1)
    assert :error = Sqlite.handle_for_shape(ctx.stack_id, shape2)
  end
  ...
  test "handle_for_shape_critical/2", ctx do
    ...
    assert {:ok, ^handle1} = Sqlite.handle_for_shape_critical(ctx.stack_id, shape1)
    assert :error = Sqlite.handle_for_shape_critical(ctx.stack_id, shape2)
  end
  ...
  test "shape_for_handle", ctx do
    ...
    assert {:ok, ^shape1} = Sqlite.shape_for_handle(ctx.stack_id, handle1)
    assert :error = Sqlite.shape_for_handle(ctx.stack_id, "no-such-handle")
  end
  ...
  test "reduce_shape_meta/3", ctx do
    expected =
      Enum.map(1..100, fn n ->
        shape = Shape.new!("items", inspector: @stub_inspector, where: "id = #{n}")
        handle = "handle-#{n}"
        {:ok, hash} = Sqlite.add_shape(ctx.stack_id, shape, handle)

        if rem(n, 2) == 0 do
          :ok = Sqlite.mark_snapshot_complete(ctx.stack_id, handle)
          {handle, hash, true}
        else
          {handle, hash, false}
        end
    ...
    assert Sqlite.reduce_shape_meta(
             ctx.stack_id,
             MapSet.new(),
  ...
  test "count_shapes/1", ctx do
    ...
    Enum.each(1..100, fn n ->
      ...
      handle = "handle-#{n}"
      {:ok, _hash} = Sqlite.add_shape(ctx.stack_id, shape, handle)
      assert {:ok, n} == Sqlite.count_shapes(ctx.stack_id)
    end)

    Enum.each(100..1//-1, fn n ->
      handle = "handle-#{n}"
      :ok = Sqlite.remove_shape(ctx.stack_id, handle)
      assert {:ok, n - 1} == Sqlite.count_shapes(ctx.stack_id)
    end)
  ...
  test "mark_snapshot_complete/2", ctx do
    assert :error = Sqlite.mark_snapshot_complete(ctx.stack_id, "no-such-handle")

    shape1 = Shape.new!("items", inspector: @stub_inspector)
    handle1 = "handle-1"
    {:ok, _hash1} = Sqlite.add_shape(ctx.stack_id, shape1, handle1)

    assert :ok = Sqlite.mark_snapshot_complete(ctx.stack_id, handle1)

    shape2 = Shape.new!("items", inspector: @stub_inspector, where: "id = 2")
    handle2 = "handle-2"
    {:ok, _hash2} = Sqlite.add_shape(ctx.stack_id, shape2, handle2)

    assert :ok = Sqlite.mark_snapshot_complete(ctx.stack_id, handle2)
  end
  ...
  test "validate_existing_shapes/1", ctx do
    ...
    {:ok, invalid_handles, 10} = Sqlite.validate_existing_shapes(ctx.stack_id)
  ...
  test "reset/1", ctx do
    ...
    assert {:ok, 10} = Sqlite.count_shapes(ctx.stack_id)

    assert :ok = Sqlite.reset(ctx.stack_id)

    assert Sqlite.reduce_shapes(ctx.stack_id, 0, fn {_handle, %Shape{} = _shape}, acc ->
             acc + 1
           end) == 0
