defmodule Electric.Postgres.ReplicationClient.MessageConverterTest do
  ...
  test "converter returns Begin change when seeing a 'Begin' message", %{
    converter: converter
  } do
    begin_msg = %LR.Begin{
      final_lsn: @test_lsn,
  ...
  test "converter returns Relation change when receiving a relation message", %{
    converter: converter
  } do
    ...
    assert %MessageConverter{relations: %{1 => @relation, 2 => ^new_relation}} = updated_converter
  end

  test "converter logs information when receiving a generic message",
  ...
  test "converter returns empty list for origin & type messages",
       %{converter: converter} do
  ...
  test "converter returns NewRecord change for insert when the relation is known", %{
    converter: converter
  } do
    {[_begin], converter} =
  ...
  test "converter returns UpdatedRecord change for update when the relation is known", %{
    converter: converter
  } do
    {[_begin], converter} =
      MessageConverter.convert(
  ...
  test "converter errors for empty old data on updates", %{converter: converter} do
    {[_begin], converter} =
      MessageConverter.convert(
        %LR.Begin{final_lsn: @test_lsn, commit_timestamp: DateTime.utc_now(), xid: 456},
        converter
  ...
  test "converter returns DeletedRecord change for delete when the relation is known", %{
    converter: converter
  } do
    {[_begin], converter} =
      MessageConverter.convert(
  ...
  test "converter returns TruncatedRelation change for truncate when the relation is known",
       %{converter: converter} do
    {[_begin], converter} =
      MessageConverter.convert(
        %LR.Begin{final_lsn: @test_lsn, commit_timestamp: DateTime.utc_now(), xid: 456},
  ...
  test "converter returns Commit change when seeing a 'Commit' message", %{
    converter: converter
  } do
    {[_begin], converter} =
      MessageConverter.convert(
