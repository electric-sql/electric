defmodule Electric.Plug.ServeShapePlugTest do
  ...
  # call/2 wraps the whole plug pipeline in an explicit try/rescue so
  # handle_errors receives the conn with full pipeline state — including the
  # before_send callback from check_admission that releases the correct
  # permit kind.
  describe "admission control release on error" do
    ...
    test "does not call release when exception occurs before check_admission runs", ctx do
      # If validation raises before check_admission, no permit was acquired and
      # no before_send callback was registered, so release must not be called.
    ...
    # Mirrors production behaviour: the Router's PassAssignToOptsPlug sets
    # conn.assigns.config before dispatching to ServeShapePlug.
