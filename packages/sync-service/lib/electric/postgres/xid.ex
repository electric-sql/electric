defmodule Electric.Postgres.Xid do
  ...
  whose value is larger in absolute terms precedes the other one (or, equivalently, the
  smaller xid belongs to a more recent transaction).
  ...
  def compare(xid64_l, xid64_r)
      when not uint32?(xid64_l) and not uint32?(xid64_r) and xid64_l > 0 and xid64_r > 0 do
    cmp(xid64_l, xid64_r)
  end
