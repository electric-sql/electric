defmodule Electric.Postgres.Configuration do
  ...
  def check_publication_for_missing_relations(conn, publication_name, relations) do
    {oids, schemas, tables} = known_relations(relations, %{}) |> IO.inspect()
    ...
    with :ok <- check_replica_identity(rows),
         :ok <- check_relations_in_publication(relations, rows) do
      :ok
    end
  end
  ...
  defp check_relations_in_publication(relations, rows) do
    ...
    if MapSet.size(diff_set) == 0 do
      ...
      error = %Electric.DbConfigurationError{
        ...
        message:
          "Database table is missing from the publication and the service has no permissions to alter it"
      }
  ...
  def configure_publication!(
    ...
    Postgrex.transaction(conn, fn conn ->
      ...
      changed_relations =
        ...
        |> IO.inspect(label: :changed_relations)

      used_filters =
        Map.drop(new_filters, changed_relations)
        |> IO.inspect(label: :used_filters)
      ...
      if pg_version < @pg_15 do
        ...
        |> IO.inspect(label: :alter_pub_set_filtered_tables)
      end
  ...
  # defp list_relation_identities(conn, relations) do
  #   {oids, schemas, tables} = relations
  ...
  #       )
  #       SELECT
  #         (ir.input_nspname, ir.input_relname) as input_relation, relreplident
  #       FROM
  #         input_relations ir
  #       JOIN
  #         pg_class pc ON pc.oid = ir.oid
