defmodule Electric.LogItems do
  ...
  @spec from_change(
          ...
          pk_cols :: [String.t()],
          update_mode :: Shape.update_mode()
        ) :: [log_item(), ...]
  def from_change(%Changes.NewRecord{} = change, txid, _, _update_mode) do
    [
      %{
        key: change.key,
        value: change.record,
  ...
  def from_change(%Changes.DeletedRecord{} = change, txid, pk_cols, _update_mode) do
    [
      %{
        key: change.key,
  ...
  def from_change(%Changes.UpdatedRecord{old_key: nil} = change, txid, pk_cols, update_mode) do
    [
      %{
        key: change.key,
        value: update_values(change, pk_cols, update_mode),
  ...
  def from_change(%Changes.UpdatedRecord{} = change, txid, pk_cols, _update_mode) do
    [
      %{
        key: change.old_key,
        value: take_pks_or_all(change.old_record, pk_cols),
  ...
  defp update_values(%{record: record, changed_columns: changed_columns}, pk_cols, :modified) do
