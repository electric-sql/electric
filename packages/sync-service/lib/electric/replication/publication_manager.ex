defmodule Electric.Replication.PublicationManager do
  ...
  def handle_call({:add_shape, shape}, from, state) do
    ...
    state = schedule_update_publication(state.update_debounce_timeout, :shape, state)
    {:noreply, state}
  end
  ...
  # This returns immediately and the actual sync is done asynchronously to
  # separate boot of shape cache from refreshing publication. This means errors
  # generated by the initial sync won't show up in the stack init, but only
  # when creating the first shape (assuming that the issue is persistent).
  #
  def handle_call(:sync_publication, _from, state) do
    state = schedule_update_publication(state.update_debounce_timeout, :sync, state)
    {:reply, :ok, state}
  end
  ...
  @impl true
  def handle_info({:update_publication, :sync}, %__MODULE__{} = state) do
    try_update_publication(&sync_publication_with_existing_shapes/1, :sync, state)
  end

  def handle_info({:update_publication, :shape}, %__MODULE__{} = state) do
    try_update_publication(&update_publication/1, :shape, state)
  end

  defp try_update_publication(apply_fun, mode, state) do
    %{prepared_relation_filters: relation_filters, retries: retries} =
      state = %{state | scheduled_updated_ref: nil}

    case apply_fun.(state) do
      {:ok, state} ->
        state = reply_to_waiters(:ok, state)
        {:noreply, %{state | committed_relation_filters: relation_filters, retries: 0}}

      {:error, err} when retries < @max_retries ->
        Logger.warning("Failed to configure publication, retrying: #{inspect(err)}")

        state =
          schedule_update_publication(@retry_timeout, mode, %{state | retries: retries + 1})
  ...
  @spec schedule_update_publication(timeout(), mode :: atom(), state()) :: state()
  defp schedule_update_publication(timeout, mode, %__MODULE__{scheduled_updated_ref: nil} = state) do
    ref = Process.send_after(self(), {:update_publication, mode}, timeout)
    %{state | scheduled_updated_ref: ref}
  end

  defp schedule_update_publication(_timeout, _mode, state), do: state
