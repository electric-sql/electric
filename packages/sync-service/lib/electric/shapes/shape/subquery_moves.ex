  ...
  defp make_move_out_pattern(
    ...
    case column_or_expr do
      column_name when is_binary(column_name) ->
        Enum.map(gone_values, &[make_value_hash(stack_id, shape_handle, &1)])
      ...
      {:hash_together, columns} ->
        ...
        Enum.map(
          gone_values,
          &[make_value_hash(stack_id, shape_handle, column_parts.(Tuple.to_list(&1)))]
        )
    end
  end
