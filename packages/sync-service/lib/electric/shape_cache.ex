...
defmodule Electric.ShapeCache do
  ...
  defp recover_shapes(state, timeout) do
    ...
    all_handles = ShapeStatus.list_shapes(state.stack_id)
  ...
  defp group_into_layers(all_handles, acc \\ [], visited \\ MapSet.new())
  defp group_into_layers([], acc, _visited), do: Enum.reverse(acc)

  defp group_into_layers(all_handles, acc, visited) do
    {appendable, missing_deps} =
      Enum.split_with(all_handles, fn {_, shape} ->
