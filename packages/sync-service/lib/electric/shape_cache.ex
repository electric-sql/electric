defmodule Electric.ShapeCache do
          ...
          :exit, {:noproc, _} ->
            ...
            cond do
              ...
              true ->
                ...
                Logger.warning(
                  "Exhausted retry attempts while waiting for a shape consumer to start initial snapshot creation",
                  shape_handle: shape_handle
                )
  ...
  def handle_continue(:wait_for_restore, state) do
    ...
    Logger.notice("Consumers ready",
      duration_ms: System.convert_time_unit(duration, :native, :millisecond),
      total_shapes: total_recovered
    )
  ...
  def handle_call({:create_or_wait_shape_handle, shape, otel_ctx}, _from, state) do
    ...
    Logger.debug("Returning shape id", shape_handle: shape_handle, shape: shape)
  ...
  defp maybe_create_shape(shape, %{stack_id: stack_id} = opts) do
    ...
    with {:ok, shape_handle} <- ShapeStatus.fetch_handle_by_shape_critical(stack_id, shape),
      ...
      :error ->
        ...
        Logger.info("Creating new shape", shape_handle: shape_handle, shape: shape)
  ...
  defp start_shape(shape_handle, shape, %{stack_id: stack_id} = opts) do
    ...
    case Shapes.DynamicConsumerSupervisor.start_shape_consumer(stack_id, %{
      ...
      {:error, _reason} = error ->
        Logger.error("Failed to start shape", shape_handle: shape_handle, error: error)
    ...
    |> case do
      ...
      {:error, failed_handle} ->
        if failed_handle != shape_handle do
          Logger.warning(
            "Failed to start consumer for handle: error starting consumer for inner shape",
            shape_handle: shape_handle,
            inner_shape_handle: failed_handle
          )
