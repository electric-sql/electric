defmodule Electric.Plug.ServeShapePlug do
  ...
  and a `try`/`after` that always releases the admission control permit.
  ...
  error-capturing frames, so the `conn` visible from the `catch` clause is the
  one passed into `call/2` — not the accumulated conn at the point of raise.
  This means we cannot rely on a `register_before_send` callback on the
  error-handler's conn for admission release. Instead, `check_admission`
  stashes the acquired permit in the process dictionary and the `after` clause
  in `call/2` releases it — firing for success, halt, and exception paths
  alike.
  """
  ...
  def call(conn, opts) do
    ...
    try do
      ...
      conn
    after
      # Must run unconditionally: OpentelemetryTelemetry keeps span contexts on
      # a per-process stack, so a missed end_telemetry_span call would leak the
      # span to the next request handled by this worker process (and grow the
      # stack over time).
  ...
  # If the response was already sent, we can't meaningfully recover, so
  # re-raise. Otherwise normalize, set a default status, and delegate to
  # handle_errors.
  ...
  defp check_admission(%Conn{assigns: %{config: config}} = conn, _) do
    ...
    case Electric.AdmissionControl.try_acquire(stack_id, kind, max_concurrent: max_concurrent) do
      :ok ->
        ...
        # uncaught exception). We can't rely on a register_before_send callback
        # here because Plug.Builder doesn't propagate the accumulated conn to
        # our rescue/catch clauses on a raise.
  ...
  #
  # Counterpart to start_telemetry_span/1: this runs near the end of call/2,
  # after all plugs (or handle_errors) have finished, so attributes captured
  # here reflect the final state of the request. The span itself is popped by
  # `OpentelemetryTelemetry.end_telemetry_span` in call/2's `after` clause —
  # keeping the pop unconditional so a span can never leak into the next
