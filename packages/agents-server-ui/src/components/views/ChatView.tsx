...
function GenericChatBody({
  ...
  // Comments are an opt-in custom collection: only show comment UI on
  // entities whose type declared a `comment` schema. On other entity
  // types the server would reject the writes anyway, so we just hide
  // the surface.
  const commentsEnabled = Boolean(
    matchingEntityTypes[0]?.custom_collection_schemas?.comment
  )
