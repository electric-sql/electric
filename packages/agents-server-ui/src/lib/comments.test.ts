...
describe(`createSendCommentAction`, () => {
  ...
  it(`posts reply metadata with the same key as the optimistic row`, async () => {
    ...
    expect(url).toBe(
      `http://localhost:4437/_electric/entities/chat/test/collections/comment`
    )
    ...
    expect(parsedBody.key).toBe(optimistic[0]!.key)
    expect(parsedBody.value).toMatchObject({
      body: `looks right`,
      from_principal: `/principal/user%3Ame`,
      reply_to: replyTo,
      target_snapshot: targetSnapshot,
    })
    expect(typeof parsedBody.value.timestamp).toBe(`string`)
  })
