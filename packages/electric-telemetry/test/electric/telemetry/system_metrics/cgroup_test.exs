defmodule ElectricTelemetry.SystemMetrics.CgroupTest do
  ...
  describe "cgroup v1" do
    setup %{tmp_dir: tmp_dir} do
      root = Path.join(tmp_dir, "v1")

      write_file(root, "memory/memory.usage_in_bytes", "2097152\n")

      write_file(root, "memory/memory.stat", """
      rss 1048576
      cache 524288
      ...
      # unlimited sentinel
      write_file(root, "memory/memory.limit_in_bytes", "9223372036854771712\n")

      # cpuacct.usage is in nanoseconds
      write_file(root, "cpuacct/cpuacct.usage", "9000000000\n")

      write_file(root, "cpu/cpu.stat", """
      nr_periods 100
      nr_throttled 7
    ...
    test "emits parsed memory metrics with v1 key mapping", %{root: root} do
      m =
        collect_cgroup_metrics(fn ->
          Cgroup.measurement(%{}, cgroup_version: :v1, cgroup_root: root)
        end)

      assert m["cgroup.memory.current"] == 2_097_152
      # v1 anon <- rss, file <- cache
      assert m["cgroup.memory.anon"] == 1_048_576
      assert m["cgroup.memory.file"] == 524_288
      assert m["cgroup.memory.working_set"] == 2_097_152 - 262_144
    end

    test "skips memory.max when v1 unlimited sentinel", %{root: root} do
      m =
        collect_cgroup_metrics(fn ->
          Cgroup.measurement(%{}, cgroup_version: :v1, cgroup_root: root)
        end)
    ...
    test "emits a real numeric v1 memory limit", %{root: root} do
      write_file(root, "memory/memory.limit_in_bytes", "1073741824\n")
      ...
      assert m["cgroup.memory.max"] == 1_073_741_824
    end

    test "converts cpu usage / throttled time from ns to us", %{root: root} do
      m =
        collect_cgroup_metrics(fn ->
          Cgroup.measurement(%{}, cgroup_version: :v1, cgroup_root: root)
        end)

      # 9_000_000_000 ns -> 9_000_000 us
      assert m["cgroup.cpu.usage_usec"] == 9_000_000
      assert m["cgroup.cpu.nr_throttled"] == 7
      # 250_000_000 ns -> 250_000 us
      assert m["cgroup.cpu.throttled_usec"] == 250_000
    end

    test "does not emit PSI on v1", %{root: root} do
      m =
        collect_cgroup_metrics(fn ->
          Cgroup.measurement(%{}, cgroup_version: :v1, cgroup_root: root)
        end)
    ...
    test "does not emit io metrics on v1", %{root: root} do
      m =
        collect_cgroup_metrics(fn ->
          Cgroup.measurement(%{}, cgroup_version: :v1, cgroup_root: root)
        end)
