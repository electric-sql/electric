...
import { createTempCertDir, skipIfNotPlatform, skipIfCannotTestTrust, ENABLE_INTEGRATION_TESTS, hasCommand } from "./test-utils"

// Safe Integration Tests - Certificate generation and validation without system modification
describe.skipIf(!ENABLE_INTEGRATION_TESTS)(`TrustInstaller safe integration`, () => {
...
describe.skipIf(skipIfCannotTestTrust())(`TrustInstaller full system integration`, () => {
  let testCertDir: string
  let installer: TrustInstaller
  let manager: CertificateManager
  ...
  describe.skipIf(skipIfNotPlatform(`darwin`))(`macOS integration`, () => {
    it(`should install and verify certificates on macOS`, async () => {
      ...
      expect(installResult.success || installResult.needsManualInstall).toBe(true)
  ...
  describe.skipIf(skipIfNotPlatform(`linux`))(`Linux integration`, () => {
    it(`should handle Linux certificate installation`, async () => {
      ...
      if (!installResult.success) {
        expect(installResult.manualInstructions).toContain(`update-ca-certificates`)
      }
    }, 30000)
  })
  ...
  describe.skipIf(skipIfNotPlatform(`win32`))(`Windows integration`, () => {
    it(`should handle Windows certificate installation`, async () => {
      ...
      if (!installResult.success) {
        expect(installResult.manualInstructions).toContain(`Certificate`)
      }
    }, 30000)
  })
})
...
describe.skipIf(!ENABLE_INTEGRATION_TESTS)(`MkcertManager integration`, () => {
  ...
  beforeEach(() => {
    testCertDir = join(process.cwd(), createTempCertDir())
    mkcertManager = new MkcertManager(testCertDir, [`localhost`], `integration-test`)
  })
  ...
  it.skipIf(!hasCommand(`mkcert`))(`should detect mkcert CA installation status`, () => {
  ...
  it.skipIf(!hasCommand(`mkcert`) || !mkcertManager.isCAInstalled())(`should generate certificates using mkcert`, async () => {
...
describe.skipIf(!ENABLE_INTEGRATION_TESTS)(`CertificateManager fallback integration`, () => {
  let testCertDir: string
  ...
  it(`should use appropriate certificate generation method`, async () => {
    ...
    const mkcertManager = new MkcertManager(testCertDir, [`localhost`], `test`)
    const expectedMethod = (mkcertManager.isAvailable() && mkcertManager.isCAInstalled()) 
      ? `mkcert` 
      : `basic-ssl`
