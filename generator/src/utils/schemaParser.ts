...
export type Field = { field: string, type: string, attributes: Array<Attribute> }
export type Model = {
  name: string,
  fields: Array<Field>
}
...
export function parseModels(prismaSchema: string): Array<Model> {
  ...
  const modelBodies = matches.map(match => match.groups! as { name: string, body: string })
...
function parseFields(body: string): Array<Field> {
  ...
  const fieldRegex = /^\s*(?<field>\w+)\s+(?<type>[\w\?]+)\s*(?<attributes>((@[\w\.]+\s*)|(@[\w\.]+\(.*\)+\s*))+)?\s*$/gm // /^\s*(?<field>\w+)\s+(?<type>[\w\?]+)\s*(?<attributes> ((@[\w\.]+\s*)|(@[\w\.]\([\w\,\.]\)+\s*))+)?\s*$/gm
  const fieldMatches = [...body.matchAll(fieldRegex)]
  const fs = fieldMatches.map(match => match.groups as { field: string, type: string, attributes?: string })
  return fs.map(f => ({
    ...f,
