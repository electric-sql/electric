...
const args = [dbName, adapter, migrator, notifier, socketFactory, console, config] as const
...
test('starting multiple satellite processes works', async t => {
  const mockRegistry = new MockRegistry()
  const s1 = await mockRegistry.startProcess('a.db', adapter, migrator, notifier, socketFactory, console, config)
  const s2 = await mockRegistry.startProcess('b.db', adapter, migrator, notifier, socketFactory, console, config)
  const s3 = await mockRegistry.startProcess('c.db', adapter, migrator, notifier, socketFactory, console, config)
...
test('ensure starting multiple satellite processes works', async t => {
  const mockRegistry = new MockRegistry()
  const s1 = await mockRegistry.ensureStarted('a.db', adapter, migrator, notifier, socketFactory, console, config)
  const s2 = await mockRegistry.ensureStarted('b.db', adapter, migrator, notifier, socketFactory, console, config)
  const s3 = await mockRegistry.ensureStarted('c.db', adapter, migrator, notifier, socketFactory, console, config)
...
test('stopAll works', async t => {
  const mockRegistry = new MockRegistry()
  const [s1, s2, s3] = await Promise.all([
    mockRegistry.ensureStarted('a.db', adapter, migrator, notifier, socketFactory, console, config),
    mockRegistry.ensureStarted('b.db', adapter, migrator, notifier, socketFactory, console, config),
    mockRegistry.ensureStarted('c.db', adapter, migrator, notifier, socketFactory, console, config)
  ])
...
test('stopAll works across running, stopping and starting', async t => {
  const mockRegistry = new MockRegistry()
  await mockRegistry.ensureStarted('a.db', adapter, migrator, notifier, socketFactory, console, config)
  await mockRegistry.ensureStarted('b.db', adapter, migrator, notifier, socketFactory, console, config)
  const p1 = mockRegistry.ensureStarted('c.db', adapter, migrator, notifier, socketFactory, console, config)
  const p2 = mockRegistry.ensureStarted('d.db', adapter, migrator, notifier, socketFactory, console, config)
